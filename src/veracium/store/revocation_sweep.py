...
carrier — the carrier-completeness rule):
  ...
  * the identity RESOLUTION and DIGEST construction beyond mirroring the
    shipped source-identity domain primitive byte-for-byte (0006 owns it, and names the domain tag — not quoted here: the sole-implementation gate greps for it, and prose that quotes a checked string defeats the check);
...
PORTED VERBATIM into the product from the normative reference
(`specs/evidence/0022/reference_revocation.py`). The acceptance
discipline is reference-vs-production AGREEMENT (the 0020 V10
precedent); a verbatim port makes the pure core exact BY CONSTRUCTION,
so every real risk lives in the adapter (`revocation.py`) that projects
the SQLite store into this module's store-dict shape and applies the
closed effect vocabulary. The differential vector test runs BOTH this
module and the reference over the vector corpus and asserts equality,
so even the copy is checked rather than trusted. Do not edit except by
re-porting.
"""
...
# (the digest DOMAIN constant lives with the sole shipped implementation in
#  source_identity.py — the port DELEGATES; see digest_of below)
...
# (_framed removed with the domain constant — the delegated digest is the
#  only consumer the reference had for it)
...
def digest_of(origin: Optional[str], source_id: Optional[str],
    ...
    if source_id is None:
        return None
    # THE ONE PERMITTED DEVIATION FROM THE VERBATIM PORT, and it is required
    # by an accepted rule rather than chosen: 0020 pins the identity digest to
    # EXACTLY ONE implementation in src/ (the second-copy class), and the
    # reference's inline construction became that second copy the moment this
    # file entered the product tree. The port DELEGATES to the shipped
    # implementation; the differential vector corpus proves the two agree, so
    # the delegation is checked rather than trusted.
    from ..scope_linkage import identity_digest_of
    return identity_digest_of(origin, source_id, local_origin)
