"""specs/0019 §4b — the shipped grounding predicate (0.10.0).

Implements EXACTLY the pinned §4b predicate: an extracted object's SPECIFICS
tokens (pure digits, alphanumeric identifiers, proper-noun runs minus the
position-0 sentence-leading single word) must each be grounded in the event
text — verbatim, or for ISO dates, by membership in the RESOLUTION SET of
the event text's date expressions against the session date (proximity
grounds nothing; longest-match precedence consumes qualified weekdays).

The NORMATIVE reference is `specs/evidence/0019/reference_predicate.py`
(pure, portable, in-repo); U3 (`test_predicate_matches_the_pinned_vectors`)
binds this module to the reference on the pinned vectors under four hash
seeds. Predicate changes are SPEC changes — the token classes, the ±366-day
window, and the grounding rule re-enter review (§4b).

Zero LLM calls; deterministic; seed-independent by construction (atomic
per-span ISO judgment by character position, R3-1).
"""
