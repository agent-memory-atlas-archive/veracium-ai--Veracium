...
The contract of a printed instruction, as this gate reads it, is the header's own three facts:
`sha256 <64 hex>`, `<N> bytes`, and a line `Here: <command>` in which `<this file>` stands for the
carrier's path. A carrier with a TERMINAL RECORD header but NO printed command is reported N/A by
name (a not-run check must never look like a pass), and the gate REFUSES if it checked nothing at all.
"""
...
HEADER = re.compile(r"<!-- TERMINAL RECORD.*?-->[ \t]*\n", re.S)   # a marker that CLOSES A LINE (trailing blanks tolerated here, NOT by a printed `^-->$`), never one quoted inside a printed command
...
def _verify(path: pathlib.Path, header: str, *, cwd: pathlib.Path = ROOT) -> tuple[str, str]:
    ...
    return "ok", f"{_name(path)}: {stated[:16]}… over {nbytes} bytes, by the printed command"
...
def test_a_record_without_a_printed_command_is_reported_not_passed(tmp_path):
    ...
    status, detail = _verify(copy, HEADER.match(copy.read_text()).group(0))
