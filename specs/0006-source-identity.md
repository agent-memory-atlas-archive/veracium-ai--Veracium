...
| **Version** | **v10 — POST-ACCEPTANCE AMENDMENT: THE STORE BOUNDARY STATED (the owner's word, dev session 2026-09-14: *"Confirm as relayed"*, on research's recommendation he ruled at 02:06Z: *"Go with your recommendation"*).** `Store` / `Store.add_edge` is an interface a host IMPLEMENTS, not a write API a host CALLS (§4 rule 10, I16): every guarantee this spec makes about a record — I1, I3, I13, I15 — is a guarantee about records written THROUGH `Memory`; a record a host writes by calling `add_edge` directly carries exactly what that host minted, and the store re-derives none of the `Memory`-level facts. Stated, not gated, on the reasoning research gave and the owner confirmed: an unsourced third-party record written that way is VISIBLE to the doctor's `sources` check (v7, verified firing with a negative control), while the silent failure of the same route — a hand-minted procedural record rendering as a declared one — got a mechanism instead (0037 v23: `Provenance` frozen, the producer stamp). The sentence stands verbatim in three carriers (this spec, 0037 v23, `docs/api.md` "Providing a store") and a test reads all three. The reason the statement cannot stand alone, recorded so the next reader does not re-derive it: it is true as INTENT and open as SURFACE — `Memory.store` is a public attribute and `SqliteStore` is exported from the package root (426 `add_edge` calls in 41 test files exercise it as a write path), so the API shape invites the very call the rule describes. Prior: **v9 — POST-ACCEPTANCE AMENDMENT: THE PROCEDURE PATH (the owner's word, dev session 2026-09-12: *"Do A and B"* on dev's scoping).** Two gaps, both EXECUTED at v0.24.0 before the word: (a) the served MCP `record_procedure` tool took `source_id` as a TOOL ARGUMENT and stored it unchanged — a model minting or impersonating a source identity, which I1 forbids, on a surface I1's test (the extractor) never reached, while 0037 §2's signature carried the argument as contract; (b) `Memory.record_procedure` with a third-party author and no `source_id` was WRITTEN with `require_source_id` on — rule 9 (v7/v8) was wired into `ingest_event` only. A: the served tool exposes NO `source_id` and takes the DEPLOYMENT's binding (`VERACIUM_MCP_SOURCE_ID`, `build_server(source_id=)`) exactly as the served `remember` does since v7; I1's domain now names both served write tools. B: rule 9 reaches `record_procedure` — a third-party-authored or declared third-party-derived procedure with no `source_id` raises `SourceIdRequired` before any write when the flag is on, the MCP tool serialising `source_id_required`. BREAKING only for an MCP host whose model set `source_id` on `record_procedure` (the tool shipped 2026-09-08; none known); the replacement is the binding. 0037 v17 is amended alongside. *Prior:* **v8 — POST-ACCEPTANCE AMENDMENT: `require_source_id` DEFAULTS ON (option C, stage 3; the owner's word, dev session 2026-09-12: *"Do  Stage 3: the flip of require_source_id to on"*).** `MemoryConfig.require_source_id` is now `True`. BREAKING for every host that ingested third-party-authored or declared third-party-derived content without a `source_id`: the library refuses with `SourceIdRequired`, the MCP tool with `source_id_required`, the CLI needs `--source-id`; an MCP deployment with no `VERACIUM_MCP_CAPABILITY` attestation carries the third-party baseline (0031 §4a) and so now needs `VERACIUM_MCP_SOURCE_ID` or every `remember` is refused. The opt-out is `MemoryConfig(require_source_id=False)`, which restores v7-and-earlier behaviour exactly; rule 8 and I3/I13 govern the records that exist either way, the `doctor` `sources` check still names them, and nothing is invented for existing rows (I1). The 0011 §4d floor stays spared. The blast radius in this repo's own suite, measured under the flipped default before any fix: 81 nodes in 18 files ingested third-party content without an id — every file that measures records-that-exist now opts out at its fixture with the reason stated; the selfcheck's third-party cell, the smoke test, the MCP routing test and every docs example supply an id instead. Rule 9 and I15 reworded; I15's opt-out node renamed. *Prior:* **v7 — 2026-09-12** — *POST-ACCEPTANCE AMENDMENT: `require_source_id` introduced (the owner's staged ruling, dev session 2026-09-12: "Go with option C"), the refusal at ingest for third-party-AUTHORED and DECLARED third-party-derived content with no `source_id` (§4 rule 9, I15), the 0011 §4d floor excluded because only the ingest site can tell a declaration from its absence, the host-set MCP binding, the CLI flag, the refusal name `source_id_required`; v7 left the flag opt-in, with the flip reserved for the owner's word.*; **v6 — ACCEPTED 2026-08-09** — *the reviewer passed `0006`'s acceptance review (round 5, no further design round). v6 = v5 + the mechanical closure edits required for the status transition, no design change: **C1** removed the superseded "no-DDL" claims (§0b/§1/Q3) and refreshed the stale External-review row; **C2** stated the origin entropy exactly (a canonical UUIDv4 from a CSPRNG, **122 random bits** — not 128, UUIDv4 fixes 6 bits); **§11 Review closure** ledger added (PROCESS.md §4a). v5 folded F3 (v4-import absent-`origin` rejected as malformed, I14) + the origin-generation/store-lineage-identity amendments; v4 folded F1/F2 (nullable digest, one shared canonical `source_identity_digest`); v3 folded R7–R11. The `0006`↔`0014` interface freeze was reviewer-signed at round 4 and is independent of `0014`'s own acceptance.* |
...
8. **🔴 No `source_id` ⇒ no source identity ⇒ no digest (reviewer F1).** `source_id` is optional and
   ...
   **complete** identities; an unknown-source contribution is *recorded* but is **not revocable by
   ...
   invariant **I13**.
9. **`require_source_id` (v7).** With `MemoryConfig.require_source_id` on, an ingest whose
   evidence is third-party-AUTHORED, or whose content is DECLARED third-party-derived
   (`EvidenceContext.derived(THIRD_PARTY)` or the legacy `derived_from=THIRD_PARTY`), MUST
   carry a `source_id`; without one it is REFUSED before the LLM runs or anything is
   written (`SourceIdRequired`; the MCP refusal `source_id_required`). The 0011 §4d floor —
   no declaration at all — is NOT refused: absence of a declaration is not a claim about
   the source. ON by default since v8 (option C stage 3, the owner's word 2026-09-12); a
   host may set it False, and rule 8 and I3/I13 govern the records that exist either way.
   Since v9 the rule reaches BOTH producers: `record_procedure` refuses a third-party-
   authored or declared third-party-derived procedure with no `source_id` the same way
   (its context is always explicit, so its derivation is a declaration). New
   invariant **I15**.
10. **The store boundary (v10).** `veracium.store.base.Store` — and `Store.add_edge` in
   particular — is **an interface a host IMPLEMENTS, not a write API a host CALLS**. The
   write API is `Memory`: `remember`, `record_procedure`, `correct`, `retire`, the import
   boundary. Rules 1–9 and every invariant in §6 describe records written THROUGH `Memory`;
   a record a host writes by calling `add_edge` directly carries exactly the identity,
   stamps and disclosure that host minted, and the store neither re-derives nor checks the
   `Memory`-level facts (the source-id requirement of rule 9 is wired into `ingest_event`
   and `record_procedure`, not into the store). The store's OWN rules still hold there — a
   stored basis, kind stamp or producer stamp never changes on a same-id replace, a
   successor of a procedural record stays procedural with the same producer (0037 §3, v23)
   — and `Provenance` is a frozen model since 0037 v23, so a stamp is minted at
   construction or not at all. This is a STATED limit, deliberately not a gate (the owner's
   ruling, 2026-09-14): an unsourced third-party record written this way is visible to the
   doctor's `sources` check; a hand-minted procedural record renders like a declared one,
   and 0037 v23 answers that with a mechanism rather than a sentence. New invariant **I16**.
...
|---|---|---|
| **I1** `source_id` is never set from model output — neither from the extractor's triples nor (v9) from a served MCP tool's arguments: both served write tools, `remember` and `record_procedure`, expose no `source_id` and carry the DEPLOYMENT's binding | `test_source_id_is_not_reachable_from_the_extractor` — drive `ingest_event` with an extractor returning `source_id` in every triple; assert it is ignored; `test_the_served_remember_tool_exposes_no_source_id`, `test_the_served_record_procedure_tool_takes_the_deployment_binding_not_an_argument` (`tests/test_0006_require_source_id.py`; skip without the optional MCP SDK) | CI |
...
| **I2b** (R9) a valid current-format IMPORT PRESERVES the file's foreign `origin`, does NOT localise it | `test_a_foreign_origin_is_preserved_not_localised` — import `(A,"mailbox:primary")` into a store of origin B; assert it stays `(A,…)` and does not group with local `(B,…)`. (I2a and I2b differ by PATH — v2 combined the two into one rule that contradicted itself, R9.) | CI |
...
| **I5** (affirmative, R1) `(origin, source_id)` affects **no** trust/authority/disclosure/staleness/supersession decision in v1 — it is only recorded, grouped, inspected | `test_source_id_changes_no_decision` — over the ladder, the gate, and staleness clearing, adding/changing `(origin, source_id)` changes no output | CI |
...
| **I14** (F3) a current-format imported record never acquires the destination `origin` by omission — a `FORMAT_VERSION` ≥ 4 record with `origin` absent is rejected as malformed (§4 rule 4), never resolved to the local singleton | `test_a_v4_import_missing_origin_is_rejected` — hand-write a `FORMAT_VERSION`-4 file with a record omitting `origin`; assert the import is REJECTED, and the record does NOT enter the store as `(local_origin, source_id)` | CI |
| **I16** (v10) the store boundary is STATED where a host reads and where the invariants it bounds live: the sentence "an interface a host IMPLEMENTS, not a write API a host CALLS" stands verbatim in this spec, in 0037, and in `docs/api.md`'s "Providing a store" section beside `add_edge`; and the mechanism that makes the statement more than a sentence holds — `Provenance` is frozen (no ordinary assignment changes a built stamp; an AST sweep finds no src site assigning to a provenance attribute) | `test_the_store_boundary_is_stated_in_both_specs_and_the_host_documentation`, `test_provenance_is_frozen_after_construction`, `test_no_src_site_assigns_to_a_provenance_attribute` (`tests/test_0037_producer_stamp.py`) | CI |
| **I15** (v7; the default since v8) with `require_source_id` on (the default), a third-party-authored or DECLARED third-party-derived ingest with no `source_id` is refused before any write (no edge, no episode, no journal event), the floor is not refused, a sourced one is accepted, and a store opted out with `require_source_id=False` behaves as v6; (v9) the same refusal on `record_procedure` for a third-party-authored or declared third-party-derived procedure | `test_require_source_id_refuses_unsourced_third_party_before_any_write`, `test_require_source_id_spares_the_floor_and_the_opted_out_store`, `test_the_mcp_source_binding_is_host_set_and_the_refusal_is_named`, `test_the_served_remember_tool_exposes_no_source_id` (skips without the optional MCP SDK; `specs/skip_inventory.py`), `test_require_source_id_reaches_record_procedure_before_any_write` | CI |
...
- **New attack surface:** (a) a **model** setting `source_id` — blocked by I1; (b) a **local caller**
  ...
  untrusted-import boundary (applied first); (d) (v10) **a host writing through `Store.add_edge`
  directly** — not gated: the store is an interface a host IMPLEMENTS, not a write API a host CALLS
  (§4 rule 10, I16); such a record carries what the host minted, the doctor's `sources` check sees
  an unsourced third-party one, and the procedural case is 0037 v23's mechanism. Content smuggled into `source_id` (F3) is diagnostic and
