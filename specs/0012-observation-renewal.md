...
> missed). v16 = that single alignment: I10k lists the registry + policy version as `0003`'s
> preserved bindings and its test retains the custom-registry invalidation case. "No other
> bin-(a) defects were found" — the protocol stands; v16 is the round-14 closure submission.
> Earlier (v15):** round 12 was the FIRST BOUNDARY ROUND: the reviewer ACCEPTED
...
| **I10k** | **(R10-2 + R11-3 + R13-1)** the wiki cache identity binds EXACTLY the §4c(iv) input→cache-effect matrix: **the functional-relation REGISTRY semantics and the contention-policy version (accepted `0003`'s existing bindings, preserved)**, plus the `0012` additions — wiki input budget, per-group variant cap, per-item cap, marker-grammar version — and does NOT bind render-time knobs (query/proactive budgets, wiki render share, `contested_members_per_line`); zero-write reuse of a stale-identity cache is forbidden, hot and cold | `test_cache_identity_binds_the_selection_policy` — knob-by-knob: each BINDING knob changed alone forces recompile hot+cold and changes the digest — **including a changed custom relation REGISTRY (the accepted-`0003` invalidation case, retained)**; each NON-BINDING knob changed alone does NOT (no spurious recompiles); a synthesized pre-v12 cache is never served without a write |
...
| **`test_reinforcement_plan_inserts_no_duplicate`** (`tests/test_0003_supersession_store.py`) | asserts only the prior remains after a reinforcement plan | inverted AND renamed at implementation: `test_reinforcement_plan_persists_the_incoming` asserts the planner builds `insert_incoming=True`/no-upsert, the incoming persists, the prior byte-untouched (an I1/I2 check) |
| **`test_reinforcement_still_advances_observed_at`** (`tests/test_staleness_clearing_0008.py`) | asserts the prior's `observed_at` ADVANCES — its own docstring already flags *"whether it should at all"* as deferred to this spec | inverted AND renamed at implementation: `test_reinforcement_no_longer_advances_observed_at` (the prior's `observed_at` must NOT move); `0008`'s C3 row records the supersession as landed |
