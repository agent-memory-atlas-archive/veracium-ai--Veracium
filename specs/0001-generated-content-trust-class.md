...
| **Decision + date** | **ACCEPTED 2026-08-25** — external round 18, with evidence-maintenance status (specs/reviews.py); v22 implemented (§32) |
| **Path** | full |
...
|---|---|---|
| **I1** assistant + **ANY** subject → `use_only` *(v4 carrier sweep: I1/I2 still encoded the withdrawn v2 subject rule; I1 is now the every-subject form)* | `test_i1_assistant_is_use_only_for_every_subject` (parametrized over user/other/assistant subjects) | CI |
...
| **I13a** *(v7, R5-4)* `SCHEMA_V11 == SCHEMA_V10` — the bump is semantic, byte-identical schema (the 0019 `SCHEMA_V7 = SCHEMA_V6` precedent, asserted not cited) | `test_i13a_schema_v11_is_byte_identical_to_v10` (object-list equality) | CI |
...
## 11. Review history — changes in v2 (after research's internal review)

*(Everything from here down is the round-by-round record. It names
artifacts that were retired when the candidate folded into the product at
acceptance — the patch, its measurement generator, its results record, its
extraction checker and the stand-in harness — because that is what those
rounds were about. They are gone from the tree on purpose; see §32.)*
...
*(PROCESS §4a — one row per review finding, with openable evidence. This
ledger is GENERATED from `specs/reviews.py` and `specs/closure_findings.py`;
edit those, then run `python3 specs/render_closure.py --write`.*

*0001 ran eighteen external rounds and was ACCEPTED at round 18 with
evidence-maintenance status. Every evidence command below runs from the repo
root under the reviewer's offline interpreter. Commands that reach a test
module importing `veracium` carry `PYTHONPATH=src`, which is how the shipped
offline launcher runs the suite — that interpreter has no `veracium`
installed, by design.*

*Rounds 11–17 closed defects in the candidate-measurement machinery, and
rounds 3–10 in the patch and the stand-in harness. Those artifacts RETIRED
when the candidate folded into the product at implementation (§32), so their
rows now run the shipped tests that carry the same property, or — where the
finding was about the machinery itself — the assertion that the retirement
happened. The fixes are recorded in this document's review history.)*

<!-- GENERATED:review-closure -->

**1 internal round(s) and 18 external round(s) with a returned VERDICT are recorded for `0001`; 16 package(s) were dispatched** — counted from `specs/reviews.py`, which is the source this block is generated from. A round appearing here and not there, or the reverse, is impossible by construction. **SENT rows are dispatch records, not outcomes**, and are labelled below so the two are never summed.

| round | date | findings raised (from `raised=`) | verdict (compressed) |
|---|---|---|---|
| internal 1 (verdict) | 2026-07-31 | 1 | revise (research) — the v1->v2 amendment set, recorded collectively: per-finding texts were folded directly into the spec (§11 lists every change) before the per-finding discipline existed; declared here under one collective id per the evidence-loss precedent (0003 rounds 3-4) |
| external 1 (verdict) | 2026-07-31 | 2 | deferred — the two verified findings, both worse than stated (§12 preserves them in full): the subject rule cannot work (no entity resolution exists; 19,096 distinct subjects, 39.4% literal user) and subject identity does not establish evidence authority (speaker != witness). v3 withdrew the widenin… |
| external 2 (verdict) | 2026-08-01 | 1 | deferred on ONE blocking amendment: the render marker keyed on use_only with hardcoded third-party text — under v3 every assistant edge would carry an affirmatively false origin. CLOSED in shipped code 2026-08-15 (graph._ORIGIN_LABELS author-keyed, fail-safe unverified-origin, test_render_origin tri… |
| external 3 (SENT) | 2026-08-22 | — | SENT (package `0001-v3` — the line's first SEALED round; rounds 1-2 were document-only sends predating the sealer, v2 deferred 2026-07-31 and v3 deferred 2026-08-01 on the render-marker gate). 0001 at v4: the generated-content trust class, re-entering on Quentin's word. v4 = v3's ruled content (ASSI… |
| external 3 (verdict) | 2026-08-23 | 7 | RETURN FOR MAJOR AMENDMENT (package `0001-v3`; the reviewer also used the archive as the C-plus live specimen — C-plus findings filed separately). Core direction coherent; six blocking + one moderate, every blocking one an EXECUTED collision between v4's promises and a contract that shipped while th… |
| external 4 (SENT) | 2026-08-23 | — | SENT (package `0001-v4`, candidate draft v5 — the round-3 fold). Every blocking finding folded at its named site (§15 maps them): the affirmation contract measured in both shapes (same value grounds via the user edge, differing value retires via the ladder — the harness found the same-value shape du… |
| external 4 (verdict) | 2026-08-23 | 4 | RETURN FOR AMENDMENT (package `0001-v4`). Four blocking. R4-1: the confirm() correction unswept across FIVE more normative carriers (§3.1, §3.2 questions, §4 twice, §7, §9) — the third partial sweep this spec has recorded. R4-2: same-value affirmation does NOT render-collapse (collapse groups per tr… |
| external 5 (SENT) | 2026-08-23 | — | SENT (package `0001-v5`, candidate draft v6 — the round-4 fold; §16 maps the findings). The confirm() sweep completed with the three-way terminology (affirmation/confirmation/never-promoted-in-place); the separate-partitions render truth stated and measured in the harness (collapse_for_render, surfa… |
| external 5 (verdict) | 2026-08-23 | 5 | RETURN FOR AMENDMENT (package `0001-v5`; C-plus C5-1/C5-2 filed separately). Four blocking + one moderate. R5-1: THREE confirm() carriers still alive after two claimed-complete sweeps — the scripted replacements had silently no-opped on wrapped text (the fold process now refuses needle misses and gr… |
| external 6 (SENT) | 2026-08-23 | — | SENT (package `0001-v6`, candidate draft v7 — the round-5 fold; §17 maps the findings). The terminology sweep executed dead with needle-miss-refusing folds; the I6 reserve precedes truncation with coverage_share=0.0 pinned; the harness drives gate.partition_parts and asserts the rendered partitions … |
| external 6 (verdict) | 2026-08-23 | 5 | RETURN FOR AMENDMENT (package `0001-v6`; C-plus C6-1/C6-2 filed separately). Four blocking + one moderate. R6-1: I13b/c undercounted the v10 domain — the executable authority accepts FIVE manifestations (constructor; constructor->v10 inline/alter; v6->v10 inline/alter) and 'both routes' lost three; … |
| external 7 (SENT) | 2026-08-23 | — | SENT (package `0001-v7`, candidate draft v8 — the round-6 fold; §18 maps the findings). I13b/c parameterized over all five v10 manifestations with the count DERIVED from accepted_digests(10); I13d requires the edge-indexed step registry, the empty-step-set digest, and generated diagnostics over base… |
| external 7 (verdict) | 2026-08-23 | 4 | RETURN FOR NARROW AMENDMENT (package `0001-v7`; C-plus C7-1/C7-2 filed separately) — NO NEW TRUST-MODEL ARCHITECTURAL DEFECT; all four blocking findings are executable-evidence quality, three of them IN the candidate patch the round asked for (which the reviewer credits with exposing them). R7-1: th… |
| external 8 (SENT) | 2026-08-23 | — | SENT (package `0001-v8`, candidate draft v9 — the round-7 fold; §19 maps the findings). The patch now implements §4b's decision order VERBATIM with the complete label matrix tested; the five-shape tests construct each accepted v10 manifestation FROM the authority's object records with v10-vs-v11 dum… |
| external 8 (verdict) | 2026-08-23 | 3 | RETURN FOR NARROW EVIDENCE AMENDMENT (package `0001-v8`; C-plus C8-1/C8-2 filed separately) — no new trust-model defect, and THE ACCEPTANCE SENTENCE: 'Once those narrow items close, I would recommend finite acceptance of 0001 v9 without reopening the frozen architecture.' R8-1: the I6 composition ve… |
| external 9 (SENT) | 2026-08-23 | — | SENT (package `0001-v9`, candidate draft v10 — the round-8 fold; §20 maps the items). The four I6 vectors assert EXACT ORDERED IDs incl. the collapse-derived dedup survivor and ranked backfill; the downgrade regression carries the parse sentinel (no record validates before the refusal — the reviewer… |
| external 9 (verdict) | 2026-08-23 | 3 | RETURN — NOT READY FOR ACCEPTANCE (package `0001-v9`; C-plus C9-1/C9-2/C9-3 filed separately, one blocker each side). The named C8/R8 attacks largely closed (reversed selection now fails three I6 tests; the version-check-after-parsing mutation now fails the parse-sentinel regression; both package sh… |
| external 10 (SENT) | 2026-08-23 | — | SENT (package `0001-v10`, candidate draft v11 — the round-9 fold; §21 maps the findings). R9-1: the I6 selection CONSTRUCTS reserved + remainder (the filter over the globally scored list is gone); the reviewer's non-functional has_pet vector added exactly as described (the one assertable record glob… |
| external 10 (verdict) | 2026-08-23 | 2 | RETURN — one blocking + one carrier; C-PLUS ACCEPTED THIS ROUND ('approve the C-plus implementation' — all three C9 closures verified, the widened/unknown/dead/stale/malformed-name/unclaimed attacks all refuse; design note §18). R10-1 (blocking): I6 does not enforce QUERY-RELEVANT — the patch reserv… |
| external 11 (verdict) | 2026-08-24 | 1 | 🏁 FINITE DESIGN ACCEPTANCE for draft v12, returned for ONE mechanical evidence-carrier correction (package `0001-v11`, sha 65c2e03a verified identical across five workspace copies; 151-command transcript, all package/identity/reconciliation/closure checks passed; clean qualified offline run 1784/21)… |
| external 11 (SENT) | 2026-08-23 | — | SENT (package `0001-v11`, candidate draft v12 — the round-10 fold; §22 maps the findings; C-plus is ACCEPTED so this package carries the 0001 line alone). R10-1: the reserve takes query-RELEVANT assertables only — the overlap bit carried from scoring (relevant_ids), the reviewer's exact bananas coun… |
| external 12 (verdict) | 2026-08-24 | 1 | FINITE DESIGN ACCEPTANCE STANDS; returned for one mechanical evidence amendment before the status flip (package `0001-v12`, sha b4ca5d56 verified; the trust-model architecture remains frozen on the complete §6 invariant surface; v13 contains NO design change; no semantic, trust-boundary, migration o… |
| external 12 (SENT) | 2026-08-24 | — | SENT (package `0001-v12`, candidate draft v13 — the round-11 fold; the reseal the finite-acceptance verdict asked for). R11-1 mechanised exactly as requested: specs/evidence/0001/measure_candidate.py applies THE SHIPPED PATCH to a clean tree at the base commit and runs both suites into candidate_res… |
| external 13 (verdict) | 2026-08-24 | 1 | RETURN for one mechanical evidence amendment; FINITE DESIGN ACCEPTANCE REMAINS IN FORCE, no new semantic or trust-model defect (package `0001-v13`, sha 49e28273 verified; archive safety, identity/header/transcript/closure all passed; candidate patch applied cleanly, focused 21 passed; offline suite … |
| external 13 (SENT) | 2026-08-24 | — | SENT (package `0001-v13`, candidate draft v14 — the round-12 fold; §24 maps it). R12-1 closed on all five requirements: the record is validated against a CLOSED exactly-typed schema (every key required at every level, unknown keys refused, every value range-checked); failure_set sorted, unique, node… |
| external 14 (verdict) | 2026-08-25 | 1 | RETURN for one mechanical amendment; FINITE DESIGN ACCEPTANCE REMAINS IN FORCE, no semantic or trust-model defect (package `0001-v14`, sha 0bfc0c77 verified; archive safety, identity, header, transcript, closure and reconciliation all passed; the full-record comparison correctly catches ALL FOUR typ… |
| external 14 (SENT) | 2026-08-24 | — | SENT (package `0001-v14`, candidate draft v15 — the round-13 fold; §25 maps it). R13-1 closed at the boundary the reviewer named: the enforcement is a NAMED injectable seam (enforce_candidate_replay) rather than inline in main(), the comparison is a PURE function (record_differences) so the replay's… |
| external 15 (verdict) | 2026-08-25 | 1 | RETURN for one mechanical amendment; FINITE DESIGN ACCEPTANCE REMAINS IN FORCE and R14-1 is CLOSED (package `0001-v15`, sha 4ba81fd3 verified; archive safety, identity, header, transcript, closure and reconciliation passed; the exact unreachable-version attack now fails; candidate patch byte-identic… |
| external 15 (SENT) | 2026-08-25 | — | SENT (package `0001-v15`, candidate draft v16 — the round-14 fold; §26 maps it). R14-1 closed exactly as specified: the test now EXECUTES main() with the current package identity, monkeypatches enforce_candidate_replay to raise a sentinel and REQUIRES it to be reached, with a second sentinel on meas… |
| external 16 (verdict) | 2026-08-25 | 1 | RETURN for one mechanical amendment; FINITE DESIGN ACCEPTANCE REMAINS IN FORCE and R15-1 is CLOSED (package `0001-v16`, sha 6dced06b verified; archive safety, identity, header, transcript, closure and reconciliation passed; removing --verify now fails its regression; candidate patch unchanged, focus… |
| external 16 (SENT) | 2026-08-25 | — | SENT (package `0001-v16`, candidate draft v17 — the round-15 fold; §27 maps it). R15-1 closed on all three requirements: the enforcement's EXACT command is asserted ([sys.executable, measure_candidate.py, --verify]) together with its working directory; the --verify branch is proven to discriminate w… |
| external 17 (verdict) | 2026-08-25 | 1 | RETURN for one mechanical amendment; FINITE DESIGN ACCEPTANCE REMAINS IN FORCE and the literal R16-1 self-copy is now caught (package `0001-v17`, sha fa1f51b2 verified; archive safety, identity, header, transcript, closure and reconciliation passed; candidate patch unchanged, focused 21/21; offline … |
| external 17 (SENT) | 2026-08-25 | — | SENT (package `0001-v17`, candidate draft v18 — the round-16 fold; §28 maps it). R16-1 closed as specified: measure(base, run=None) takes an injectable subprocess seam, and test_the_measure_producer_derives_the_record_from_real_commands drives it with canned outputs deliberately UNLIKE the shipped r… |
| external 18 (SENT) | 2026-08-25 | — | SENT (package `0001-v18`, candidate draft v20 — the round-17 fold at §29, PLUS the TERMINUS NOTE at §30 as an in-archive member; the 2101Z seal of this same version is disclosed discarded-unsent, since the note must ship WITH the package that asks its question rather than following by a side channel… |
| external 18 (verdict) | 2026-08-25 | 2 | 🏁 0001 ACCEPTED WITH EVIDENCE-MAINTENANCE STATUS (package `0001-v18`, sha afeea5b5 verified) — the finite design acceptance from round 11 STANDS, implementation and the status flip may proceed, and NO FURTHER BLOCKING SPECIFICATION ROUND is warranted. The terminus question is granted under its optio… |

**Per-finding closure ledger — PROCESS §4a.** **46 finding(s) for `0001`** — every number here is DERIVED from the rows below (external round 7, R7-1: the manifest claimed 26 while the ledgers held 31, and 0023 said 9/9 above a 10-row table); the total across the tracked specs is derived once, in `specs/STATUS.md`. Generated from `specs/closure_findings.py` and validated against `specs/reviews.py` on `(spec, kind, round, id)` EXACTLY — extras, duplicates, wrong rounds and empty evidence all fail the build.

| finding | round | what it was | closed in | evidence (runnable) |
|---|---|---|---|---|
| **0001-R8-1** | external 8 | the I6 vectors asserted membership, not order — a reversed selection passed all four | exact ordered-ID equality in every vector, incl. the collapse-derived dedup survivor | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i6_composition_reserved_day_overlap tests/test_0001_generated_content_trust.py::test_i6_composition_distinct_reserved_days tests/test_0001_generated_content_trust.py::test_i6_composition_dedup_across_reserve_and_coverage tests/test_0001_generated_content_trust.py::test_i6_composition_underfill_backfills_by_rank_deterministically -q -p no:randomly` |
| **0001-R8-2** | external 8 | the downgrade test could not detect a version-check-after-parsing regression (its enum still knew ASSISTANT) | the parse sentinel: Edge/Episode.model_validate trapped; refusal must precede any record validation | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i13_pre_assistant_reader_refuses_a_v11_store -q -p no:randomly` |
| **EVIDENCE-M18-1** | external 18 | the production/test-double divergence survived R17-1: the implementation was only ever CALLED with an injected runner, so a branch keyed on the runner's identity sent every test down the honest path and production down a read of the shipped record — and --verify agreed, because agreement is a question a record can answer about itself | the seal-time enforcement perturbs the record in exactly one field and REQUIRES the replay to contradict it there and nowhere else, which no record-reading implementation can do; plus an injection-free regression running the producer with its external commands unreachable, which must FAIL rather than report. Both mutants planted — the reviewer's exact form caught by both gates, the git-guarded form by the seal-time probe | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member -q -p no:randomly  # the discrimination probe and the injection-free production binding RETIRED with the candidate they guarded when 0001 was implemented; this asserts that retirement actually happened, which is the only claim about them that can still be true` |
| **PACKAGE-M18-1** | external 18 | the archive-membership test required git metadata, so it FAILED in the extracted package — the one environment where membership is a fact rather than a prediction; and the reason no gate caught it is 0022 R9-1 recurring in the carrier that fix did not reach: the launcher, the only check running the whole qualified suite, ran at cwd=ROOT and measured the build tree while the header presented that number as the package's | the membership test is extraction-aware (tracked-implies-member in the tree, presence-IS-membership in an extraction) and the qualified suite is now the last EXTRACTION_CHECKS entry, run inside the extracted archive, where a red run REFUSES THE SEAL | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member tests/test_spec_gate.py::test_the_extraction_check_list_matches_the_sealer_registry -q -p no:randomly` |
| **0001-R17-1** | external 17 | the injected path was bound and the PRODUCTION path was not — an optional runner made them two paths, so a no-runner branch returning the shipped record passed every test while production --verify compared the record with itself | the runner is REQUIRED on the implementation and measure() is a delegation and nothing else; a sentinel regression proves the wrapper reaches the implementation with the requested base and subprocess.run; three production-path mutants planted and failing | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member -q -p no:randomly  # this finding was closed against the candidate-measurement machinery, which RETIRED when 0001 was implemented and the candidate folded into the product; the surviving checkable claim is that the retirement happened, which is what this asserts. The fix itself is recorded above and in the spec's review history.` |
| **0001-R16-1** | external 16 | the PRODUCER was unbound — every consumer was tested while measure() was only ever monkeypatched, so a body of `return json.loads(RECORD.read_text())` made the verifier compare the record with itself and pass in a non-git extraction | measure() takes an injectable subprocess seam; a behavioural regression proves the record is DERIVED from canned command output unlike the shipped figures, the declared base materialised and the patch applied; three collapse mutants planted and failing | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member -q -p no:randomly  # this finding was closed against the candidate-measurement machinery, which RETIRED when 0001 was implemented and the candidate folded into the product; the surviving checkable claim is that the retirement happened, which is what this asserts. The fix itself is recorded above and in the spec's review history.` |
| **0001-R15-1** | external 15 | the chain was tested link by link but never at the join — dropping --verify from the argv ran the helper's default measure-and-print mode, which exits 0 without comparing, with the whole gate green | the exact invocation and cwd are asserted; the --verify branch is proven to discriminate on a planted difference with an identical control; the default mode's exit-0 is documented by assertion; three connection-breaking mutants planted and each failing | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member -q -p no:randomly  # this finding was closed against the candidate-measurement machinery, which RETIRED when 0001 was implemented and the candidate folded into the product; the surviving checkable claim is that the retirement happened, which is what this asserts. The fix itself is recorded above and in the spec's review history.` |
| **0001-R14-1** | external 14 | reachability was proved syntactically — an AST search rejecting only a literal constant-false guard, so `if a.version == 'v0'` disabled replay for every real package with the whole gate green | the test EXECUTES main() and requires a sentinel to be reached, with a second sentinel naming the bypass; the call is an unconditional fail-fast precondition; three bypass shapes planted and each verified failing | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member -q -p no:randomly  # this finding was closed against the candidate-measurement machinery, which RETIRED when 0001 was implemented and the candidate folded into the product; the surviving checkable claim is that the retirement happened, which is what this asserts. The fix itself is recorded above and in the spec's review history.` |
| **0001-R13-1** | external 13 | the seal-time replay was sound but UNPROTECTED — a planted `if False` on its call left the named matrix and the whole spec gate green, so the only guard that catches type-valid fabrications could be removed silently | the enforcement is a named injectable seam and the comparison a pure function; a sealer-boundary regression binds reachability (if-False and deletion both planted and both failing), refusal on a failing replay with a pristine control, and discrimination including failure-identity replacement | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member -q -p no:randomly  # this finding was closed against the candidate-measurement machinery, which RETIRED when 0001 was implemented and the candidate folded into the product; the surviving checkable claim is that the retirement happened, which is what this asserts. The fix itself is recorded above and in the spec's review history.` |
| **0001-R12-1** | external 12 | the checker bound a PROJECTION — patch hash, README count, README triple, failure LENGTH — while the record claims base commit, environment, commands, focused outcome and a sorted failure set; a forty-zero base with python 0.0.0, and a duplicated failure entry, both exited 0 | a closed exactly-typed schema validating every claimed field; failure_set sorted/unique/node-id/cardinality-equal; commands imported not retyped; the matrix across every field; and the base independently reproducible by a sealer-run complete-record replay | `$PY -m pytest tests/test_collected_header.py::test_the_terminus_note_is_an_archive_member -q -p no:randomly  # this finding was closed against the candidate-measurement machinery, which RETIRED when 0001 was implemented and the candidate folded into the product; the surviving checkable claim is that the retirement happened, which is what this asserts. The fix itself is recorded above and in the spec's review history.` |
| **0001-R11-1** | external 11 | the candidate README stated a focused count of 20 while the branch ran 21 — carried from v10 and incremented by inference, inside a paragraph claiming the measurement was re-run | both figures are GENERATED into candidate_results.json by running the shipped patch, and bound to the patch's bytes and the README's text by a checker in the sealer's extraction checks | `$PY -m pytest tests/test_spec_gate.py::test_the_extraction_check_list_matches_the_sealer_registry -q -p no:randomly  # the candidate README, its generator and its results record RETIRED when the candidate folded into the product at acceptance; the property they enforced — a package's figures are MEASURED from the artifact, never typed — is carried now by the extracted-suite gate in the sealer registry, which this binds. (P4 governs this row, so it names one test and runs under the reviewer's bare offline interpreter.)` |
| **0001-R10-1** | external 10 | the I6 reserve protected ELIGIBILITY, not relevance — every assertable in the scored set was reservable, and user-subject edges sit there at baseline score with zero query overlap; the reviewer's executed counterexample reserved an unrelated 'bananas' fact first and dropped a relevant one | the relevance bit is carried FROM scoring (relevant_ids) and the reserve takes query-relevant assertables only; the exact bananas vector added and proven to fail pre-fix; the I6 cell says query-RELEVANT explicitly | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i6_no_relevant_assertable_reserves_nothing -q -p no:randomly` |
| **0001-R10-2** | external 10 | the opening block still read 'draft (v10)' with a §20 pointer beside the v11 Version row — the third version-carrier strike | the opening block carries NO revision; the Version row is structurally the one carrier; the sweep greps '(v' forms | `! sed -n '1,20p' specs/0001-generated-content-trust-class.md \| grep 'draft (v'  # the opening block carries no revision (the §22 changelog QUOTES the old defect, so the sweep is scoped to the block)` |
| **0001-R9-1** | external 9 | subgraph_for_query FILTERED the globally scored list instead of constructing reserved + remainder — a low-ranked reserved assertable record under a non-functional relation surfaced last, and every order vector used functional works_as (top-ranked reserves), masking it | the output is CONSTRUCTED as reserved + remainder (both segments in scored order); the reviewer's has_pet vector asserts the complete ordered output and fails on the pre-fix filter; the dedup vector asserts full order, closing the §20 claim | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i6_reserved_low_rank_is_placed_first_nonfunctional tests/test_0001_generated_content_trust.py::test_i6_composition_dedup_across_reserve_and_coverage -q -p no:randomly` |
| **0001-R9-2** | external 9 | the candidate test module docstring still said candidate/0001-v8 — one version carrier survived the R8-3 sweep | the module docstring is version-neutral; the sweep now greps the whole patch | `! grep -nE 'candidate/0001-v[0-9]+' tests/test_0001_generated_content_trust.py` |
| **0001-R9-3** | external 9 | the README's full-suite measurement was stale (16 failed/1797 vs the extracted branch's 16 failed/1787/21 skipped) and carried no environment | the measurement is re-run at packaging time and recorded with its exact environment (python, platform, command) | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py -q -p no:randomly` |
| **0001-R8-3** | external 8 | the modified patch still identified as candidate/0001-v8 while the draft moved to v9 | version-neutral artifact (candidate.patch); the Version row is the one carrier | `! grep -nE 'candidate/0001-v[0-9]+' tests/test_0001_generated_content_trust.py` |
| **0001-R7-1** | external 7 | the candidate patch implemented and tested the WRONG I12 label; USER/SYSTEM inherited a label | the §4b decision order verbatim; the complete matrix tested | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i12_the_complete_label_matrix tests/test_render_origin.py -q -p no:randomly` |
| **0001-R7-2** | external 7 | the five-manifestation tests were proxies: routes not shapes, v11-vs-v11, count-only inheritance, a hardcoded 5, a fabricated reader | shapes from the authority's own object records; digest-level inheritance; the qualified head-10 reader | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i13b_stamp_only_across_every_accepted_v10_shape tests/test_0001_generated_content_trust.py::test_i13c_v11_inherits_by_digest_not_count -q -p no:randomly` |
| **0001-R7-3** | external 7 | I7's test was a constant assertion; the spec-named downgrade test was absent | test_downgrade_export_fails_cleanly, real, both import modes | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_downgrade_export_fails_cleanly -q -p no:randomly` |
| **0001-R7-4** | external 7 | the four I6 composition branches were never executed | four vectors with exact-ID/order assertions, all green against the unchanged reserve implementation | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i6_composition_reserved_day_overlap tests/test_0001_generated_content_trust.py::test_i6_composition_distinct_reserved_days tests/test_0001_generated_content_trust.py::test_i6_composition_dedup_across_reserve_and_coverage tests/test_0001_generated_content_trust.py::test_i6_composition_underfill_backfills_by_rank_deterministically -q -p no:randomly` |
| **0001-R6-1** | external 6 | I13b/c undercounted the v10 domain: five accepted manifestations, 'both routes' lost three | I13b/c parameterized over every accepted shape; the count derived, not typed | `grep -n 'EVERY accepted v10 manifestation\\|inherits_every_v10_shape' specs/0001-generated-content-trust-class.md` |
| **0001-R6-2** | external 6 | the 0018 attestation internals were undispositioned: migration_digest from ALTERS_V7_TO_V8, ladder diagnostics hard-coding v5/v6-era bases | I13d: edge-indexed step registry, empty-step-set digest, generated diagnostics, bases 1-10 | `grep -n 'edge-indexed migration-step registry' specs/0001-generated-content-trust-class.md` |
| **0001-R6-3** | external 6 | I5 named collapse_for_render while the design said it neither partitions nor renders | I5 names gate.partition_parts + rendering; collapse retained only as the no-collapse assertion | `grep -n 'asserted at .gate.partition_parts' specs/0001-generated-content-trust-class.md` |
| **0001-R6-4** | external 6 | the reserve/coverage composition was untested beyond share=0.0 | the composition defined; the harness measures the positive-coverage path | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i6_composition_reserved_day_overlap tests/test_0001_generated_content_trust.py::test_i6_composition_distinct_reserved_days -q -p no:randomly` |
| **0001-R6-5** | external 6 | a live cross-ref said 'the confirm() row' — the sweep grepped rule phrases, not row names | fixed; cross-references join the sweep list | `! sed '/GENERATED:review-closure/,$d' specs/0001-generated-content-trust-class.md \| grep -n 'see the .confirm(). row'` |
| **0001-R5-1** | external 5 | three confirm() carriers survived two claimed-complete sweeps — the scripted replacements silently no-opped on wrapped text | all three replaced; folds refuse needle misses; zero survivors grep-verified | `! sed '/GENERATED:review-closure/,$d' specs/0001-generated-content-trust-class.md \| grep -n 'Only through .confirm()' && ! sed '/GENERATED:review-closure/,$d' specs/0001-generated-content-trust-class.md \| grep -n 'confirm()..-class'` |
| **0001-R5-2** | external 5 | I6's post-_cover reserve was impossible — truncation precedes it; assertable_selected 0 measured at the shipped coverage_share=0.0 | the reserve applies to the full scored set BEFORE truncation; the fixture pins coverage_share=0.0 | `grep -n 'BEFORE final truncation' specs/0001-generated-content-trust-class.md && PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i6_reserve_guarantees_the_user_edge -q -p no:randomly` |
| **0001-R5-3** | external 5 | I5 tested collapse_for_render, which neither partitions nor renders | the harness drives gate.partition_parts and asserts grounded-only / unverified-only / origin marker / no leakage | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i5_affirmation_grounds_and_partitions_and_confirm_refuses -q -p no:randomly` |
| **0001-R5-4** | external 5 | the v10->v11 contract was prose-only in the new-reader direction | I13a-d name the executable checks | `grep -c 'I13[abcd]' specs/0001-generated-content-trust-class.md` |
| **0001-R5-5** | external 5 | current-version carriers disagreed: Status row, the §9 label, the harness self-id | swept; version carriers join the pre-send sweep | `$PY -c "import re, pathlib; s = pathlib.Path('specs/0001-generated-content-trust-class.md').read_text(); ver = re.search(r'[*][*]Version[*][*] . [*][*](v\\d+)[*][*]', s).group(1); assert f'{ver} is the round-' in s, 'Status row disagrees with Version'; h = pathlib.Path('tests/test_0001_generated_content_trust.py').read_text(); assert not re.search(r'v\\d+ candidate harness', h), 'the shipped suite carries a version literal'; print('version carriers consistent, no hand-bumped literals')"` |
| **0001-R4-1** | external 4 | the confirm() correction was unswept across five more normative carriers — the third partial sweep this spec has recorded | §3.1/§3.2/§4/§7/§9 swept with the three-way terminology | `! sed '/GENERATED:review-closure/,$d' specs/0001-generated-content-trust-class.md \| grep -n 'Promotion remains' && sed '/GENERATED:review-closure/,$d' specs/0001-generated-content-trust-class.md \| grep -c 'AFFIRMATION\\|affirmation'` |
| **0001-R4-2** | external 4 | same-value affirmation does not render-collapse: collapse groups per trust envelope, so the spec claimed a collapse 0012 forbids | the separate-partitions truth stated; the harness measures the RENDERED result | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i5_affirmation_grounds_and_partitions_and_confirm_refuses -q -p no:randomly` |
| **0001-R4-3** | external 4 | I6 claimed scope runs upstream — false against shipped 0020: scope filters AFTER selection, and a principal's edge was starved at cap 1 | I6 scoped to unscoped recall; the scoped limitation in §8; the 0020 amendment at Q6 | `grep -n 'scoped to UNSCOPED recall' specs/0001-generated-content-trust-class.md && grep -n 'Q6' specs/0001-generated-content-trust-class.md` |
| **0001-R4-4** | external 4 | the v10->v11 contract was incomplete: wrong precedent, missing 0013/0018 dependencies, the release orchestrator undispositioned, the refusal untyped, the Q3 carrier contradicting I13 | §7 completed; I13 exact; the harness asserts StoreVersionError(reason=newer) | `grep -n 'StoreVersionError' specs/0001-generated-content-trust-class.md && PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i13_pre_assistant_reader_refuses_a_v11_store -q -p no:randomly` |
| **0001-INT1** | internal 1 | research's v1->v2 amendment set, folded directly into the spec before the per-finding discipline existed | §11 (changes in v2), the collective record | `grep -n '## 11. Review history — changes in v2' specs/0001-generated-content-trust-class.md` |
| **0001-R1-SUBJECT** | external 1 | the subject rule cannot work: no entity resolution, no display name; 19,096 distinct subjects, 39.4% the literal 'user' | withdrawn in v3 — use_only for EVERY subject, no subject inspection | `grep -n 'One rule, no subject inspection' specs/0001-generated-content-trust-class.md` |
| **0001-R1-WITNESS** | external 1 | subject identity does not establish evidence authority — speaker is not witness; 'the deploy failed' proves nothing about who deployed | conceded in v3; groundability routed to the evidence-basis axis | `grep -n 'Speaker . witness, conceded' specs/0001-generated-content-trust-class.md` |
| **0001-R2-MARKER** | external 2 | the render marker keyed on use_only with hardcoded third-party text — every assistant edge would carry an affirmatively false origin | closed in shipped code 2026-08-15: author-keyed _ORIGIN_LABELS, fail-safe unverified-origin, the tripwire test | `PYTHONPATH=src $PY -m pytest tests/test_render_origin.py::test_every_author_reaching_use_only_has_a_deliberate_label -q -p no:randomly  # the original check asserted that USER, SYSTEM and THIRD_PARTY each had an entry in the author-keyed label map. 0001 I12 made the label PAIR-keyed and deliberately REMOVED the USER and SYSTEM entries, because inheriting another class's origin string is the very failure this finding was about — so the old assertion now demands the defect. The property it protected is unchanged and is checked over the whole author x derivation matrix against an independent oracle, including that no unlabelled class is described as a third party's or the assistant's.` |
| **0001-R3-1** | external 3 | confirm_edge (0008) refuses every non-assertable edge by contract, so the promotion v4 promised cannot exist; affirmation is NEW USER evidence — same value grounds via the user edge, differing value retires via the ladder | §3.2 affirmation row, I5, §7, §8; both shapes measured | `PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i5_affirmation_grounds_and_partitions_and_confirm_refuses -q -p no:randomly` |
| **0001-R3-2** | external 3 | the on-disk guard was never activated: disk=10 reader=10 gives ValidationError mid-read, not a refusal — 0007 only refuses what a version bump tells it to | §7 (SCHEMA 10->11 semantic bump), I13 refusal-at-open | `grep -n 'I13' specs/0001-generated-content-trust-class.md && PYTHONPATH=src $PY -m pytest tests/test_0001_generated_content_trust.py::test_i13a_schema_v11_is_byte_identical_to_v10 tests/test_0001_generated_content_trust.py::test_i13_pre_assistant_reader_refuses_a_v11_store -q -p no:randomly` |
| **0001-R3-3** | external 3 | two matrix cells measured false (cross-class absorption stays blocked; 0012 persists restatements untouched) and the supersession rationale named the wrong mechanism | §3.2 rewritten against the ladder/0012/0.4.1 as shipped; I3b, I10a | `PYTHONPATH=src $PY -m pytest tests/test_absorption.py::test_third_party_restatement_never_absorbs_user_fact tests/test_0001_generated_content_trust.py::test_i1_assistant_is_use_only_for_every_subject -q -p no:randomly` |
| **0001-R3-4** | external 3 | I6 named a test with no selection rule — selected=40 user_selected=[] under the natural fixture | I6: the reserve rule (min(count_relevant_assertable, ceil(budget/4))) with the exact 1,000+1 fixture | `grep -n 'THE SELECTION RULE' specs/0001-generated-content-trust-class.md` |
| **0001-R3-5** | external 3 | Q4 rode ingest._source_type, deleted by 0016 — the currency pass re-ran commands but never verified cited names resolve | Q4 struck as moot; the consumer row corrected | `! grep -rn '_source_type' src/veracium/ingest.py && grep -n 'MOOT (v5, R3-5)' specs/0001-generated-content-trust-class.md` |
| **0001-R3-6** | external 3 | three more v2-form carriers survived the v4 sweep: the Edge.subject blast-radius cell, §3b bullet 4, §8's public claim | §2/§3b/§8 rewritten; §14's completed-sweep claim corrected by §15 | `grep -c 'v5, R3-6' specs/0001-generated-content-trust-class.md  # all three carriers annotated at the fix site` |
| **0001-R3-7** | external 3 | Spec-Requires named 0003/0005 while the design depends on 0007/0008/0012/0016 and explicitly sequences after 0024 | the header line, complete | `grep -n 'Spec-Requires: 0003, 0005, 0007, 0008, 0012, 0013, 0016, 0018, 0024' specs/0001-generated-content-trust-class.md  # grown again by R4-4 (+0013 +0018); the closure tracks the CURRENT list` |

<!-- /GENERATED:review-closure -->

## 32. The evidence scaffold retired (v22, 2026-08-26)

0001 is implemented. The candidate patch was applied to main, the sixteen
carrier failures it measured were closed, and the machinery that existed
to evidence an unimplemented spec is gone from the tree: the patch, its
measurement generator, its results record, its extraction checker, and
the stand-in harness.

This is the retirement the terminus note described, and it is worth
recording that the note's *account* of it was right while its *scope* was
narrower than the note first claimed — the sealing path skipped the
machinery gracefully, but the producer refused rather than skipping, and
the reviewer's PACKAGE-M18-1 found a defect in the note's own membership
test. Both are now moot in the same way: the artifacts are gone.

What replaced them, and why it is not a loss of coverage:

* the stand-in harness constructed `author=USER, derived=THIRD_PARTY`
  because `ASSISTANT` did not exist when it was written. The shipped
  suite uses REAL `ASSISTANT` records, which is strictly better evidence
  for a spec about assistant-authored content — the proxy was always the
  thing being apologised for.
* twenty closure rows cited the patch or the harness, ten of them by
  `grep`. Every one now runs a shipped test. That was P4's whole point:
  a grep for a diagnostic string is satisfied by a no-op artifact
  containing the string.
* the seal-time candidate replay retired with the record it replayed.
  The general gate that outlived it — the qualified suite run inside the
  EXTRACTED archive — is not candidate-specific and stays.

The one thing deliberately kept is `specs/evidence/0001/TERMINUS-NOTE.md`,
with the verdict recorded at its head. It is the record of what was asked
and on what grounds, and its test now asserts that this retirement
happened rather than that the code to perform it still exists.
