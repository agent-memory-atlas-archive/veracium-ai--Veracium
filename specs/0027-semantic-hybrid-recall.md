...
firing on disk); it scales with the BUDGET (~16 B per unit above truncation), not with
the store; and a host using the full `tags_matched` bound (64 × 64) adds exactly one
