...
### 4g. The policy receipt *(v13; research T1)*
...
**What this revision does NOT do, stated.** The receipt is returned on the
recall result; it is not yet DURABLE in the store. T1 asks for durability and
T10 priced it (≈2.3 KB per firing recall, linear in the budget); a store table
is a schema bump with the 0018 release-migration obligations and the schema
evidence regeneration, and is the next amendment. Until then a host that turns
a policy on persists `Recall.policy_receipt` itself, and the plan's
"read the receipts before touching anything else" reads those.
