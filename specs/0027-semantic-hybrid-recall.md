...
oracle (0040 §4), and the query itself would be a new content carrier 0040's
...
about a person, which is 0040's class for `evidence_ref` and `source_id` and
is stated in 0040 §2b-ii-bis as such — and the identifier alphabet is the
...
column is a blob carrier 0040 enumerates beside `edges.json`, with one
...
row per firing recall (T10's ≈2.3 KB, linear in the budget) and nothing here
prunes it — research T9's class, with erasure per user the only remover.
Redaction: 0040's D4 (tombstone the receipt on a redacted supersession; a
