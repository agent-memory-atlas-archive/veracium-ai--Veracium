...
> Rounds 11 to 13 removed every `NAME = declare_site(...)` and its census imports. They then refused, one spelling at a
> time, each route by which other code could still reach the removed name: an import, a star, `__all__`, an attribute,
> `getattr`, `import_module`, `sys.modules`, `globals()`. The round-13 verdict found five more — `pkgutil.resolve_name`,
> `runpy.run_module`, `importlib.util` loading, a function's `__globals__` and `inspect.getmodule` — and each derived a
...
> true (`tests/test_0042_inv7.py::test_r14_k2_a_twin_arm_that_touches_a_stand_in_fails_the_harness_gate`). The surface
> is pinned against the REAL class, never a list
> (`tests/test_0042_inv7.py::test_r14_k1_the_stand_in_answers_every_member_the_real_site_defines`). A stand-in missing a
...
> counted neighbour counts (`tests/test_0042_inv7.py::test_r14_the_named_residual_is_uncounted_and_its_neighbours_count`).
