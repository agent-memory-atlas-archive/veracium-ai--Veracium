...
> AST, in both directions, plus the class header. **Any change to `Site`'s OWN definition is drift**: a member's body,
> signature, defaults or decorators; a member added or removed; `__slots__`; a class-body statement; the class docstring;
> the class header (bases, decorators, keywords). Each has a refusing cell, except the class header's decorators and keywords:
> the comparison covers them with the rest of the header, and the header's cell adds a base class. A formatting-only change
> (a comment and blank lines) is not drift, and the real tree reads no drift. See
> `tests/test_0042_inv7.py::test_r16_a_drifted_site_is_refused_and_t_must_advance`.
>
...
> runs in the reference arm is counted by the census-entry hook. **Equal member ASTs build equal members** because T's
> `Site` evaluates no module-level name when the class is created: nothing in its header, class-level assignments, method
> defaults or decorators (research measured this at T, and a test pinning it is queued). A future T whose `Site` does
> evaluate one would need that premise re-established.
>
