...
| **Decision + date** | **ACCEPTED 2026-08-22** (round 12, the v7 surface) and **A1 ACCEPTED 2026-08-24** — external round 24, package `0024-0025-v24` sha `63bfcc2a…`: the amended U1–U7 surface with uniform-USE_ONLY U2 is the finite-acceptance boundary; later checker defects are evidence maintenance. Implementation re-authorised as amended |
| **Path** | full |
...
**2 internal round(s) and 24 external round(s) with a returned VERDICT are recorded for `0024`; 24 package(s) were dispatched** — counted from `specs/reviews.py`, which is the source this block is generated from. A round appearing here and not there, or the reverse, is impossible by construction. **SENT rows are dispatch records, not outcomes**, and are labelled below so the two are never summed.
...
| **EVIDENCE-M24-1** | external 24 | the space-positive control was a tautology ('!= 0 or True') — the reviewer broke the closer grammar to [\t]* and the complete advertised matrix still passed | explicit == 0 assertions for space AND tab positives; the reviewer's [\t]* grammar mutant planted as a self-test proving the control can fail | `$PY -m pytest tests/test_collected_header.py::test_a1_carrier_checker_mutation_matrix -q -p no:randomly` |
| **PROCESS-M24-1** | external 24 | P1/P4 claimed 'no unmutated checker ships' while establishing a pointer convention and an invocation shape — a docstring-only reference and '$PY script && grep' both passed | the documented claims narrowed to what the gates establish (convention + shape; the kill evidence is the matrix tests CI runs) in PROCESS.md and the gate docstrings | `$PY -m pytest tests/test_spec_gate.py::test_every_evidence_artifact_declares_a_mutation_matrix -q -p no:randomly` |
| **A1-R23-1** | external 23 | the fence closer used Python strip() — U+00A0 and other Unicode whitespace closed a fence CommonMark keeps open; and the oracle's vertical-tab cell then exposed str.splitlines() breaking on \v/\f where CommonMark does not | the closer requires spaces/tabs exactly; the parser splits on true newlines only; the five-suffix whitespace oracle + tab positive control are matrix cells | `$PY -m pytest tests/test_collected_header.py::test_a1_carrier_checker_mutation_matrix -q -p no:randomly` |
...
## 23. A1 ACCEPTED (external round 24, 2026-08-24)

Verdict verbatim: *"0024 is approved for acceptance on the frozen U1–U7
surface, including revised U2. This is the finite-acceptance boundary:
subsequent checker defects are evidence maintenance and do not reopen
the design."* Terminus ruling: proposal 2 — accept with evidence
maintenance, resting on the long-stable architecture and executable
U-surface, not on P1/P4 being airtight. The three co-owned `0025`
replacements (§7b) folded into `0025` atomically with this status
change, closing the co-owned interface. All round-23 findings closed.
Two NONBLOCKING maintenance findings recorded and folded under the
maintenance regime: EVIDENCE-M24-1 (the space-positive control was a
tautology — replaced with explicit assertions and the reviewer's
`[\t]*` grammar mutant planted as a self-test) and PROCESS-M24-1 (the
P1/P4 documentation now claims what the gates establish — the pointer
convention and invocation shape; the kill evidence is the matrix tests
themselves, which CI runs).
