...
bucket, names both causes, and on a store created after the stamp existed a
non-zero count is the live reading that something wrote past the `Memory`
boundary (v23.1). The stamp is immutable on a same-id replace and inherited across a
...
4. **Gate 1 — normalise, then substring, then ONE WHOLE SENTENCE (V-WHOLE-
   ...
   span's BOUNDARIES are validated against the event, and since v24 (round-4
   findings 1a, 1b and 2 — one root cause: v22 tested CHARACTERS, not
   boundaries) a boundary is a KIND read from the complete event by
   `boundary_kind`, total over what can follow a terminal run and fail-closed:
   end-of-text, or whitespace then an uppercase letter, a digit or an opening
   quote/bracket, is a BOUNDARY; a `.` between two digits is INSIDE a sentence
   (`$100.50`); `?` anywhere in the run makes the sentence a QUESTION, refused
   whatever the span carries, and per SENTENCE — never per event, since a user
   turn that ends in a question to the assistant still yields its routine
   sentence; everything else is AMBIGUOUS and refused rather than guessed: an
   ellipsis, a single-letter or listed abbreviation before the run (`a.m.`,
   `approx.`, `Dr.`, `reqs.`), a vowel-less short token before it (`mgmt.`), a
   letter or symbol right after it (`daily.archive`), whitespace then a
   lowercase letter (`daily. archive`). The span begins at a boundary (never
   mid-token: `$100.`|`50 daily`) and ends at the end of the event, at its own
   terminal run, or immediately before a plain `.` or `!` it omitted — never
   before an omitted `?` (F1a: dropping it turned a question into a statement).
...
| **V-WHOLE-SENTENCE** (v22; round-3 findings 2 and 3; v24, round-4 findings 1a/1b/2 — boundaries are KINDS) the quoted span is exactly one whole sentence of the event: it begins at a BOUNDARY (the start of the event, or a terminal run followed by whitespace and an uppercase letter, digit or opening quote — never mid-token) and ends at the end of the event, at its own terminal run, or immediately before a plain `.`/`!` it omitted — never before an omitted `?`; a `.` between digits is inside a sentence; a run carrying `?` is a question and refuses whatever the span carries, per sentence and never per event; an ellipsis, a single-letter, listed or vowel-less-short abbreviation before the run, a letter or symbol right after it, or whitespace then a lowercase letter is an AMBIGUOUS join and refuses; the kinds are total (every shape has one; the unrecognised is ambiguous). A fragment inside a framing sentence, a fragment starting or stopping mid-sentence, two sentences however joined, and a sentence that is a question all refuse before the grammar; the whole `$100.50` sentence, the first of two sentences that start like sentences, a routine before a question to the assistant, and a sentence in the middle of a paragraph admit and store as written; mutants: accept an omitted `?`, treat a decimal point as a boundary, admit `daily.archive`, admit a lowercase continuation, read `doesn't.` as the letter `t` | `test_the_round_3_passage_selections_are_refused_end_to_end`, `test_a_whole_sentence_of_the_event_is_admitted_and_stored_as_written`, `test_the_first_sentence_admits_while_the_declared_second_instruction_is_not_carried`, `test_the_round_4_boundary_and_head_cases_are_refused_end_to_end`, `test_the_round_4_controls_admit_and_store_the_sentence_as_written`, `test_the_boundary_kinds_are_total_and_fail_closed`, `test_the_open_residual_is_pinned_so_a_change_is_noticed` (`tests/test_0037_capture.py`) | CI |
...
corpus sha256: 6d777f67e82561683e262cd0f9f4b9433ffcc0a353fa0d6ae0bcff39e53b9778
...
**Two residuals of the v24 boundary kinds, stated separately because they are
not the same kind of thing (research's read, 2026-09-14).** THE OPEN ONE, which
loses meaning: the abbreviation list is a HAND-LIST and lags the language, so
finding 1's class REMAINS REACHABLE through an abbreviation outside the list
that carries a vowel — "I review invoices per sched. Every week I archive them."
stores "I review invoices per sched" and the frequency is lost. The list carries
the business and developer registers this product is aimed at (`reqs.`, `eng.`,
`mgmt.`, `acct.`, `org.`, …; tokens that are often a sentence's last word —
`min`, `max`, `dev`, `ops`, `doc`, `spec`, `prod`, `misc`, `temp` — are left out on
purpose) and a vowel-less short token counts as an abbreviation by shape; the broad narrowing — every short unlisted word before a
period and an uppercase word — was built and measured first and refused the
canonical routine shape ("every time. Unrelated…"), so it was not taken. The
residual is pinned (`test_the_open_residual_is_pinned_so_a_change_is_noticed`)
so the day it closes or widens is noticed, and the reviewer weighs it as what it
is: meaning loss still reachable, not a tidy edge. THE CLOSED ONE, which loses
capture: a legitimate sentence beginning with a lowercase token (`pytest`,
`npm`, `iPhone`, `macOS`) makes the join before it ambiguous and the FIRST
sentence refuses too — a yield cost in exactly this product's corpus, taken in
the safe direction and stated with those examples. And one cost this version
SURFACED rather than introduced: `figure`, `guess` and `reckon` have been in the
cognitive class since v20, so "I figure out the totals every morning." refuses;
pinned, not changed.
