...
3. **Gate 0 — whose event.** Only an event whose AUTHOR is the user
   ...
   cannot capture one.
4. **Gate 1 — normalise, then substring.** Inner-whitespace collapse ONLY (no
   casefold, no punctuation strip: verbatim means verbatim; the collapse
   tolerates line wrapping) on the span, the gloss and the event text; the
   span must be non-empty and a substring of the event text. A paraphrase
   fails.
...
7. **Gate 4 — the gloss is DERIVED from the span (V-GLOSS-DERIVED; round-1
   finding 1, round-2 finding 1; the owner's word, 2026-09-13).** The model's
   `object` for a procedural triple is NOT an input and is discarded. The
   stored gloss — what `object` holds and describe renders — is the verified
   span transformed by a closed rule and nothing else: (i) whitespace-
   normalise; (ii) cut a comma-introduced RELATIVE clause (", which/who/whom/
   whose/where …") from its comma to the end of the span; (iii) whitespace-
   normalise again. No contraction is expanded, no subject is dropped, nothing
   is inflected — the stored text is the user's sentence. REJECTED BY
   MEASUREMENT before choosing: dropping the subject "I" (with the closed
   contraction table expanded first, because a bare drop strands "'ve been…")
   turns "I always run the linter before merging" into an imperative-shaped
   fragment that describe's frozen recognition rule (§8's floor) WITHHOLDS as
   `executable_detail` — measured on the reviewer's cases and the observed
   positives, the feature would render nothing for a present-simple routine;
   the sentence kept, describe renders it. The summary contract (Gate 2) is then applied to the
   DERIVED text. WHY: v19 checked the model's gloss as an ordered subsequence of
   the span and the reviewer showed that order is not meaning (a negation lost
   to a curly apostrophe; "Reviews receipts" from "I review invoices and
   archive receipts"); a predicate-level grounding (head verb, object head,
   order within the predicate, polarity, no commentary source) was built,
   measured and deleted before landing — a fourth syntactic approximation of a
   semantic property, with its own stated costs (ditransitives, adverb
   insertion, comma-coordinated object lists). Derivation eliminates the class:
   polarity, action–object relationships and conditions are the user's own
   because the words are. WHAT IT COSTS, printed rather than argued — the
   sixteen clean routines observed across 400 sessions, derived (pinned in
   `tests/eval/extraction_speech_act/derived_glosses_observed_positives.jsonl`,
   regenerated by test): "I've been misting my fern every other day"; "I go to
   the gym on Tuesdays, Thursdays, and Saturdays."; "I usually meal prep on
   Sundays and bring healthy snacks to work" (its ", which has been great" cut);
   ...
   discriminator (cut at ", and/but/so/yet" only when a NEW SUBJECT — it/that/
   this — follows; never when a verb continues the practice) cleans it and
   keeps the six coordinated-verb and list shapes; it is NOT built, by this
   spec's own rule that a marker added to close one observed case is fitted to
   that case — a second instance from another draw licenses it. Stored
   commentary is the user's own words, not a fabrication.
   WHAT IT CHANGES ABOUT THE HAZARD: span boundaries now determine
   stored text entirely, and the span is chosen by the model; a paraphrase
   used to drop trailing commentary, derivation stores whatever the span
   carries minus rule (ii). And THE GRAMMAR IS NOW THE ONLY FILTER between a
   user's sentence and stored text (Gate 3), which is why Gate 3 refuses a span
   crossing a sentence boundary and every shape it does not enumerate.
...
**How the capture surface has been maintained (v21; research's sentence, kept
because a reviewer trusting the instrument should know).** FOUR SUCCESSIVE GATE
DESIGNS HAVE BEEN RETIRED BY MEASUREMENT, none by argument: the substring gate
...
floor withheld every present-simple fragment it produced). What stands — the
positive-form grammar with its refusal classes, and a derived gloss that is the
user's sentence with one cut — is the design that has so far survived the
measurements that retired the others, and each measurement's threshold was
fixed before its run. Closed by removing a mechanism, in every case but the
first.
