...
4. **Gate 1 — normalise, then substring, then ONE WHOLE SENTENCE (V-WHOLE-
   ...
   nothing (as the `!` form never did), "Any tips? I review…" does.
...
| **V-INHERITANCE-AT-IMPORT** (v24.1; round-5 finding 3; v24.2, round-6 finding 2: the lookup sees REFUSED predecessors) the import boundary applies V-STAMP-INHERITED and V-PRODUCER-INHERITED to every incoming record against its predecessor — looked up by id among the RAW incoming records, before any refusal, or the destination's existing records, so the check is independent of file order and a predecessor refused as procedural on the default path still constrains its successors; procedural-ness is by LINEAGE (a record whose predecessor is procedural by lineage is procedural whatever markers it carries, so a chain that drops the markers one hop later is refused one hop later too; a cycle fails closed) — before anything commits, on both paths: a successor that drops the markers, or names a different producer or none where the predecessor has one, is refused per record as `inheritance_violation` with the signal and the predecessor named and counted; matching producers restore; a declarative lineage imports whole | `test_restore_applies_the_inheritance_rules_the_store_applies_to_a_write` (both orders), `test_inheritance_is_checked_against_an_existing_destination_predecessor_on_both_paths`, `test_the_default_path_keeps_a_refused_predecessor_for_its_successors_inheritance` (both orders, a two-hop chain) (`tests/test_0037_producer_stamp.py`) | CI |
...
corpus sha256: 6a9d78a348ef45f74c3f494c30176a72e00a3972665630ca43d9095745c61adf
...
30b8e1dc432f3b94, differs from research's original (43b48ffe9cfe5f2b) in
one line: its absolute local `src` path is resolved from the file — prints
the per-chunk sentence histogram both figures derive from; it needs the
local-only LongMemEval file and does not run in CI): **53.6% of chunks (1,809 of 3,372) hold more than one
