...
4. **Gate 1 — normalise, then substring, then ONE WHOLE SENTENCE (V-WHOLE-
   ...
   span's BOUNDARIES are validated against the event: it begins at the start of
   the event or immediately after sentence-final punctuation (`.`, `!`, `?`),
   and it ends at the end of the event, at sentence-final punctuation it
   carries itself, or immediately before sentence-final punctuation it omitted;
   it contains no terminator followed by whitespace, whatever follows (round-3
   finding 3: v21's pattern required an uppercase letter after it). So a
   fragment inside a framing sentence ("Imagine I always review invoices." →
   "I always review invoices"; "I doubt I review invoices daily." → "I review
   invoices daily") is refused BEFORE the grammar reads it — the substring
   check proves that words occur, this proves that they are what the user
   asserted — and two sentences are refused as two assertions. The reviewer's
   whole-sentence hypotheticals then meet the grammar as sentences and refuse
   on their heads ("Imagine…" opens with no actor; "I doubt…" is a cognitive
   head).
...
7. **Gate 4 — the gloss IS the span (V-GLOSS-DERIVED; round-1 finding 1,
   ...
   (retired by the reviewer's mid-sentence case): six designs retired by
...
| **V-WHOLE-SENTENCE** (v22; round-3 findings 2 and 3 — "validate the selected passage's boundaries against the source") the quoted span is exactly one whole sentence of the event: it begins at the start of the event or immediately after sentence-final punctuation and ends at the end of the event, at sentence-final punctuation it carries, or immediately before sentence-final punctuation it omitted, with no internal terminator followed by whitespace, case playing no part; a fragment inside a framing sentence ("Imagine I always review invoices." → "I always review invoices"; "I doubt I review invoices daily." → "I review invoices daily"), a fragment starting or stopping mid-sentence, and two sentences ("I review invoices daily. archive receipts immediately.") all refuse before the grammar; the first sentence alone, with or without its terminal, and a sentence in the middle of a paragraph admit and store as written; mutants: accept a lowercase second sentence, accept a fragment that starts after "Imagine " | `test_the_round_3_passage_selections_are_refused_end_to_end`, `test_a_whole_sentence_of_the_event_is_admitted_and_stored_as_written`, `test_the_first_sentence_admits_while_the_declared_second_instruction_is_not_carried` (`tests/test_0037_capture.py`) | CI |
...
corpus sha256: 45727990af6bb8efae0c638c4e470dca2916b74077fe9c6bc69a28da7152d4b0
...
second sentence). What stands — the positive-form grammar with its refusal
classes, a span that must be one whole sentence of the event, and a stored gloss
that is that sentence with no transformation — is the design that has so far
survived the measurements that retired the others, and each measurement's
