...
4. **Gate 1 — normalise, then substring, then ONE WHOLE SENTENCE (V-WHOLE-
   ...
   dropping it turned a question into a statement).
...
| **V-INHERITANCE-AT-IMPORT** (v24.1; round-5 finding 3) the import boundary applies V-STAMP-INHERITED and V-PRODUCER-INHERITED to every incoming record against its predecessor — looked up by id among the incoming records or the destination's existing records, so the check is independent of file order — before anything commits, on both paths: a successor of a procedural predecessor that drops the markers, or names a different producer or none where the predecessor has one, is refused per record as `inheritance_violation` with the signal and the predecessor named and counted; matching producers restore; a declarative successor of a declarative predecessor imports | `test_restore_applies_the_inheritance_rules_the_store_applies_to_a_write` (both orders), `test_inheritance_is_checked_against_an_existing_destination_predecessor_on_both_paths` (`tests/test_0037_producer_stamp.py`) | CI |
...
corpus sha256: 454fa6105f55c1ba25341e4175621e6b584cbc6213267a54da06d06f17aaf43d
...
it sits on — research measured the exposure over the raw corpus (500
sessions, 2,591 user turns, 3,372 `?`/`!`-delimited chunks): 1,563 chunks
(46.4%) are one sentence, where a routine is capturable; 1,809 (53.6%) carry
two or more, where it is not — sentence POSITIONS, not routines, an upper-ish
bound; and both of draw 5's admissions fall (0 of 6 positive rows, 0 of 4
distinct positive passages; the 16 pinned observed positives are single-span
quotes and stay 13 admitted either way, which is why they could not inform
the decision). Extractor capture now reaches a routine stated as the whole
