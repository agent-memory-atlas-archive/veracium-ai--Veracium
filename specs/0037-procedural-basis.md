...
6. **Gate 3 — the user as the ACTOR, now (V-ACTOR-PRESENT; round-1 finding 2,
   research's nine constructions and its red team).** The span must OPEN with
   the user as the actor in the present or habitual: an optional leading
   temporal or conditional clause, then the first-person SINGULAR pronoun
   (research §5: "we" is a policy, not the user), an optional auxiliary
   ("I've", "I'm", "I have", "I am"), optional "been", an optional frequency
   adverb, a word. It carries NO marker of REPORT ("told me", "says",
...
in the coordination ledger: at least 7 of 9 admitted, 0 of 22 admitted. RESULT:
8 of 9 admitted (the miss is "I've been trying to recognize bird calls" —
"trying to" is an aspiration marker); **0 of the 17 assistant instructions, 0 of
...
that follows is the owner's, not this revision's, and both arms are stated:
...
asymmetry that decided `correct()`. A third draw is the earliest measurement that
licenses either. **The realistic capture yield of this feature is small:** with
~11 clean user-stated routines per 200 sessions AND a gate that must refuse
...
roughly 11 clean user-stated routines across 200 LongMemEval sessions (9 in the
first hundred, 2 in the second) — the §8 census's conclusion arriving a third
time, from a corpus drawn to test something else. **Attack #2 occurred
...
| **V-ACTOR-PRESENT** (v19) the quoted span opens with the user as the ACTOR in the present or habitual (first-person SINGULAR, an optional leading temporal/conditional clause, optional auxiliary and frequency adverb) and carries no report, rejection, aspect, norm, request, aspiration, question, one-time or past marker; its left context in the event ends in no quotation or attribution frame; research's nine constructions, the reviewer's one-time instruction, the red team's quoted-third-party-in-first-person, first-person-plural and aspiration cases all refuse; the positive set (the ablation's routines and the v16 fixtures) admits; the 66-item bare-procedural pool admits none; the labelled 31 measure recall 8/9 with the two borderlines admitted and stated as the residual; mutants: drop the left-context check (the phishing quote admits), drop the past check ("I just used…" admits), admit "we" | `test_the_actor_present_grammar_refuses_reports_rejections_aspect_norms_and_requests`, `test_a_declared_one_time_instruction_is_refused_not_stored_as_a_procedure`, `test_the_red_team_constructions_are_refused`, `test_the_grammar_and_the_grounding_admit_the_users_own_stated_routines`, `test_the_bare_procedural_pool_never_passes_the_grammar`, `test_the_grammar_against_the_labelled_ablation_quotes_meets_the_threshold_fixed_beforehand` (`tests/test_0037_capture.py`) | CI |
...
corpus sha256: 458dfa025dcff162f04ed957a7b78c70b456e26b2e1e61b7e213b82763949b9a
