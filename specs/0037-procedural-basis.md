...
6. **Gate 3 — the user as the ACTOR, now (V-ACTOR-PRESENT; round-1 finding 2,
   ...
   inflection meets it. The SPAN's head decides, never a later clause's
...
7. **Gate 4 — the gloss DESCRIBES the span (V-GLOSS-GROUNDED; round-1 finding
   1: "I drink tea after lunch" quoted, "reviews invoices regularly" stored).**
   After a symmetric light stem and stopword removal, every content token of
   the gloss occurs in the span (containment — a gloss may not ADD), in the
   same order CLAUSE-WISE (both texts split at punctuation and at
   subordinators; a fronted clause passes, a reversal inside a clause fails —
   research §3), the gloss carries every NEGATION the span carries ("I never
   delete the logs…" may not become "deletes the logs") and every
   SUBORDINATOR the span carries ("…when testing locally" may not become an
   unconditional practice — research §4: containment alone is the unsafe
   direction out of the same test), and frequency phrases ("every Friday") are
   position-free. An empty gloss fails.
...
9. **What is written, and what is NOT (research's red team).** One `Edge` under
   ...
   carrier). No stored field holds the payload, by construction. The episode is
...
| **V-NO-CORRECTION-OF-PROCEDURES** (v19) `correct()` on a procedural record refuses with the named reason `correction_of_procedure` and writes nothing — a correction's successor would carry the host's text under the user's `stated`; end to end: a captured procedure, one `correct()`, and recall's context carries neither the gloss nor the span, and no stored field holds the span | `test_correcting_a_captured_procedure_is_refused_and_the_quote_never_renders` (`tests/test_0037_capture.py`) | CI |
...
corpus sha256: caa8152c7df2f9ac94aa2001859771b5c100d5bd1681eee7a37789c0c4d54bee
