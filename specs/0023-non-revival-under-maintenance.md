...
| # | question | state |
|---|---|---|
| **Q1** | should a host be able to choose REFUSAL instead of quarantine-at-birth for a revoked source's writes? | `resolved` — **NO HOST CHOICE IN V1; quarantine-at-birth stands alone. Dev + research jointly, 2026-08-21 (dev recommended, research CONCURRED), at implementation start.** The grounds, in research's ranking: **(1) refusal DESTROYS the record of what a revoked source attempted** — the operator investigating a compromise needs exactly that trail, and retention was §1's rejected-alternative reasoning at acceptance; **(2) the asymmetry is option-preserving** — a host wanting refusal can drop quarantined writes downstream, while recovering refused writes is impossible; **(3) the frozen surface N1–N15 was reviewed on quarantine semantics** — a second mode doubles the cell matrix with no reviewed row for the refusal half. **And the strongest ground was already IN the accepted text: §3b's policy-independence rule** — a rule about what the store ACCEPTS cannot be a per-process setting, or an honest host and a careless one produce different stores. A host-configurable refusal mode is precisely that per-process acceptance setting, so the closure is not "declined for now": the accepted design argues against it. If a real host need materialises, refusal-as-mode needs its own spec round AND must overcome §3b (likely a store-level declared mode, that round's problem). **Guidance bound (research):** the drop-downstream workaround must not migrate into docs as a recommended pattern without its cost stated — it forfeits the audit trail, and quarantine-retention IS the feature; §8's existing answer (a high-volume compromised connector should be disconnected, not absorbed) remains the answer to the host who would have wanted refusal |
...
| **Q4** | should the audit line for a quarantined-at-birth write carry the digest, or only a count? | **RESOLVED 2026-08-22 (dev, per the recorded leaning; the 2026-08-22 spec-table audit found this pre-release question had shipped in 0.13.0 unresolved AND its carrier unimplemented — both closed together): THE DIGEST, plus the count.** The ingest result carries `quarantined_at_birth` and `birth_revocation_digest` (always present; None/0 off the revoked path); the AuditLog is the consumer; telemetry's whitelist drops both; the MCP tool result strips both. The operator investigating a compromise is the whole audience, and the digest is content-free |
...
| external 17 (verdict) | 2026-08-19 | 0 | SEMANTICALLY CLEAR; deferred with the shared package findings and the atomic dependency on 0022 |
| external 17 (SENT) | 2026-08-19 | — | SENT (the coupled round-17 package `0022-0023-v17`). 0023 at v18: no semantic change required |
| external 18 (verdict) | 2026-08-19 | 0 | SEMANTICALLY CLEAR; deferred with the shared package findings and the atomic dependency on 0022 |
| external 18 (SENT) | 2026-08-19 | — | SENT (the coupled round-18 package `0022-0023-v18`). 0023 at v19: no semantic change required |
| external 19 (verdict) | 2026-08-20 | 0 | SEMANTICALLY CLEAR; deferred with the shared package findings and the atomic dependency on 0022 |
