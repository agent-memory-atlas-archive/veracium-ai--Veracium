...
- **`txn` is the atomicity carrier (F3), allocated at the DATABASE level
  ...
  **`BEGIN IMMEDIATE`**, and only then are `max(txn)` and `max(seq)`
  ...
  lock BEFORE its reads (schema_version.py:1501). A `database is locked` condition **at EITHER lock site — the
  transaction's START or its COMMIT** — retries or refuses the WHOLE
  transaction under the 0007 §4c `busy_timeout` discipline, which is stated
  THERE over the CONDITION and carries no site restriction: bounded wait, then
  a loud typed refusal **naming the site**; never a partially-allocated batch,
  never a re-used clock read. **On refusal the batch is NOT APPLIED and the
  connection is left reusable** (V-LOCK-REFUSAL-FORM, §6). This covers the
  transaction `_write_txn` OPENED; when it JOINS a caller-opened transaction
  the caller owns the commit and this guarantee does not reach it (§4a-i). An
...
### 4a-i. The lock-refusal surface — `_write_txn` OWNS the commit (v11)

**v10 specified this section over a mechanism that does not run.** It named the
covered set as *"the commit `_write_txn` itself issues, for transactions it
opened"*. **That set is EMPTY in the shipped tree.** All six
`with … self._write_txn()` bodies issue their own `self._conn.commit()` as part
of the body — `add_edge`, `confirm_edge`, `invalidate_edge`,
`apply_supersession_plan`, `commit_outcome_import_plan`, `forget_user` — so by
the time the context manager's `else` branch runs, `in_transaction` is already
False and **its commit never executes**. Measured, not read: tracing a real
`add_edge`, the guard line executes and **the commit line does not**. A wrapper
placed there catches nothing, which is exactly what the implementation found.

**v9's own sentence was already false of half of itself:** `_write_txn` is
documented as committing *"or rolls back the WHOLE batch"*. The rollback half
is real; **the commit half has never fired on any shipped path.**

#### The rule this revision now states

**`_write_txn` OWNS the commit.** No `_write_txn` body issues its own — the
commit belongs to the context manager's exit, in the same transaction, over the
same content, with `_journal_scope` unchanged. The covered set is then
**non-empty by construction** rather than by hope.

| | |
|---|---|
| **covered** | every commit that ends a transaction `_write_txn` opened — issued at ONE place, its exit |
| **NOT covered** | the same logical write when `_write_txn` **JOINS** a caller-opened transaction: it returns early on `self._conn.in_transaction`, the caller owns that commit, and this guarantee does not reach it |
| **residue, NAMED not implied** | the `.commit()` sites outside `_write_txn` paths, censused by §6. **None is wrapped for a commit-time lock failure**; whether 0007 §4c's site-independent rule obliges them is §10's open item, owed to 0007 |

#### What moving the commit changes, stated per body

**Three of the six bodies commit as their last statement and move without
consequence** (`add_edge`, `invalidate_edge`, `forget_user`). **The other three
do not. Each was resolved by execution or by reading the site — none by
assumption:**

- **`confirm_edge`** commits inside a `try` whose `except sqlite3.IntegrityError`
  handles the C8 concurrent-duplicate race on `UNIQUE(user_id, correlation_id)`,
  rolling back and returning the committed original. Moving the commit out would
  move it beyond that handler — **admissible only if the race surfaces at the
  INSERT and not at the COMMIT. MEASURED on the shipped store:** a second
  confirmation with the same `(user_id, correlation_id)` inside `BEGIN
  IMMEDIATE` raises `sqlite3.IntegrityError: UNIQUE constraint failed:
  confirmations.user_id, confirmations.correlation_id` **at the INSERT
  statement; the COMMIT is never reached.** So the C8 handler keeps its entire
  scope, and the only thing that moves is the commit-time lock — which is
  precisely V-LOCK-REFUSAL-FORM's. `test_the_duplicate_race_raises_at_the_insert_not_the_commit`
  pins it, and it is a **standing** condition: if that ever ceases to hold, this
  design does not hold either.
- **`apply_supersession_plan`** commits inside `try … except Exception:
  rollback; raise`, which **duplicates what `_write_txn` already does** —
  rollback on any `BaseException`, then re-raise. With the commit moved, the
  body's handler **becomes a deletion, not a change of behaviour**, and there is
  ONE commit-time handler where there were two.
- **`commit_outcome_import_plan`** returns counts computed **after** its commit
  today. Every value in that dict is known **before** the commit, so the
  `return` moves **after** the `with` block: a reordering with no semantic
  change, and the caller now receives counts only once the commit has actually
  succeeded. *(Belt and braces, verified by execution rather than assumed: even
  had the return stayed inside the block, a `__exit__` that raises discards the
  pending return value and propagates — a caller can never be handed counts for
  a write that did not land. `test_a_failed_commit_never_returns_import_counts`
  pins the property either way.)*

**No count appears in this section on purpose.** The covered set and the
residue are **censused** by §6, never typed here — a number written into a spec
is a hand list that goes stale against the code it describes, which is the
defect this revision exists to refuse one level down.
...
| **V-APPEND** no code path updates or deletes an event except erasure; `seq` strictly monotone per user — **amended by accepted 0041 §4c/§11.4 (2026-09-19): the redaction's in-transaction body `_redact_in_txn` (shared by `redact` and, since 0041 tranche 4b, the import's notice application) is the one UPDATE path and tombstones `state` only, on the redacted edge's prior events** | `test_event_log_is_append_only_and_monotone` (binds the updater's name and its statement's shape) | CI |
...
| **V-LOCK-REFUSAL-FORM** a `database is locked` condition arising in a transaction `_write_txn` OPENED is refused in ONE form at either lock site — the wrapped, spec-citing refusal NAMING which site failed — never bare SQLite text. Postconditions on refusal: **(a)** the exception names the lock site; **(b)** no part of the batch is applied; **(c)** `in_transaction` is False and the connection is reusable. **`_write_txn` OWNS the commit (§4a-i); no body issues its own**, so the covered set is non-empty by construction | `test_commit_time_lock_refusal_has_the_begin_time_form`; `test_the_begin_time_refusal_is_unchanged` (control — the change reached only the other site); `test_the_store_is_unchanged_and_the_next_write_lands`; **`test_no_write_txn_body_issues_its_own_commit`** (the CENSUS — walks the AST for `.commit()` inside any `with … _write_txn()` block and asserts ZERO, so the covered set is measured and moves with the code); `test_the_duplicate_race_raises_at_the_insert_not_the_commit`; `test_a_failed_commit_discards_the_import_counts`. **Mutants: a wrapper at BEGIN alone; a wrapper that SWALLOWS rather than refuses; ONE IN-BODY COMMIT REINTRODUCED — which is the state shipping today and which made v10's covered set empty** | CI |
...
| **V-KIND** the kind vocabulary is closed and derived; `invalidated` events validate `reason` against `DISPOSITIONED_REASONS` (all SEVEN); an unregistered reason refuses the write — **amended by accepted 0041 §4c/§11.4 (2026-09-19): `redacted` is the sixth kind, the one beside `invalidated` whose `reason` is non-NULL (redaction's own vocabulary)** | `test_event_kinds_closed_and_reasons_authoritative` | CI |
...
- **CROSS-SPEC, owed to 0007 (v10): does 0007 §4c's rule oblige the OTHER
  `.commit()` sites?** §4c states the loud-refusal discipline over the
  CONDITION (`database is locked`) with **no site restriction**, and §4e
  already knows *"the commit can still fail afterwards"*. This spec restores
  that scope **for the journaled edge write transaction only**. Whether the
  remaining sites inherit the obligation is **0007's question** — deciding it
  inside a 0029 revision would either widen a spec that does not own them or
  read as ruling them exempt. Stated OPEN; §6's census records the residue so
  whoever takes it starts from a measured number.
- **Retry hint (v10):** should the refusal message carry one? A caller
  obligation is not a postcondition, so *"the caller must retry"* is
  deliberately NOT in V-LOCK-REFUSAL-FORM. Whether the TEXT should advise a
  retry is a product question.
