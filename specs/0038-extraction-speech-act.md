...
- **0037** — procedural records and the `basis` axis. **This spec consumes 0037 directly and v2 never said so (round-1 R1-5):** §1a's cause B rested, at v1–v6.0, on **V-EXTRACTOR-BLIND** keeping every `relation_kind="procedural"` relation out of the vocabulary rendered to the prompt, which is why an instruction had nowhere legal to go; since 0037 v16 (**V-EXTRACTOR-QUOTE-GATED**) a ROUTINE the user states has one — the quote-gated procedural triple — and an instruction that is not a routine still has none, because 0037's V-ACTOR-PRESENT refuses it before the carrier exists (v6.2: the reviewer's imperative "Please review the invoices today"; v6.3: the first-person request "I request that you review invoices today", refused by 0037 v21's directive-head and second-person rules — both are tests); `record_procedure` is the governed path this spec declines to duplicate; and §2b's drop is correct BECAUSE the procedural carrier it exempts has passed every 0037 §4a-iii gate (v6.3 rewrites v6.2's "correct ONLY while 0037's blindness holds", a sentence about the pre-v16 world). **0037 is ACCEPTED** (external round 5, 2026-09-07). If any 0037 gate is relaxed, §2b must be re-decided — V-NO-PROCEDURAL-IN-PROMPT-VOCAB fails first and says so.
...
**V-EXTRACTOR-BLIND** kept every relation with `relation_kind="procedural"`
**out** of it (v1–v6.0; measured then) — `record_procedure` was the one governed
path for a practice. *v6.1: 0037 v16 renders the procedural relation and admits
a triple under it ONLY with a verbatim `quote` ingest verifies against the event
text (V-EXTRACTOR-QUOTE-GATED); a stated ROUTINE now has a governed place; an
instruction that is not a routine still has none because 0037's actor-present
grammar refuses it (v6.2; positive-form with directive and second-person refusals since v20/v21).* So an instruction has nowhere legal to go, and the
...
**THE PROCEDURAL-CARRIER EXEMPTION (v6.1; 0037 §4a-iii).** A stated routine is
expected to appear in `instructions` AND, since 0037 v16, as one triple under a
relation of kind `procedural` whose `quote` is the verbatim span the user stated
it in. The refusal above EXEMPTS a triple under a procedural relation that PASSED
EVERY gate of 0037 §4a-iii (v6.2: author, verbatim substring, the summary
contract, the actor-present grammar, the grounding; v6.3: the grammar positive-form
with directive and second-person refusals, and the gloss DERIVED from the span — the
model's summary discarded, nothing to ground; v6.4: the span ONE WHOLE SENTENCE of the
event, boundaries validated against the source, so a two-sentence carrier never reaches
this exemption and the stored text is the sentence as written; v6.5: the boundaries are
KINDS read from the complete event — an AMBIGUOUS join, two sentences with no space
between them or a lowercase continuation, is refused rather than admitted as one carrier,
so a declared instruction riding a second sentence is carried in neither reading; v6.6:
a boundary is ESTABLISHED at both ends or the passage is declined — the start or end of
the turn or a `?`/`!` run before a sentence start; a mid-text period establishes nothing
and no list stands in — so nothing period-separated from the carrier reaches it) — keyed on the relation's
KIND, never on the author or the text, the same shape as the `third_party_claim`
exemption. A procedural triple refused by any of those gates (counted
`procedural_refused`) never reaches this rule — in particular an imperative or
one-time instruction, or a first-person request ("I request that you…"), which the
actor-present grammar refuses; a DECLARATIVE triple restating
an instruction is refused exactly as before. The reference prompt's RULES block
directs a routine the user follows to `instructions` verbatim AND to one
procedural triple carrying the exact span; every other triple restating an
instruction remains refused.
...
| **V-NO-PROCEDURAL-IN-PROMPT-VOCAB** (REWRITTEN v6.2; v6.3) | cause B's premise, with its MECHANISM named: the vocabulary handed to the prompt carries the registered procedural relation (0037 v16), and a triple under it reaches storage only through 0037 §4a-iii's ordered gates — in particular V-ACTOR-PRESENT, which since 0037 v20 ADMITS ONLY an enumerated head shape with an action head (present simple, present perfect continuous, progressive with a frequency marker) and since v21 refuses a DIRECTIVE head and any SECOND PERSON in the span: the imperative "Please review the invoices today" (round 1) has no admitting shape, and the first-person request "I request that you review invoices today" (round 2) is refused by class and by its addressee; so an instruction that is not a routine has no storage route BECAUSE that gate refuses it, and §2b's refusal stands for declarative triples. The residual is 0037's, carried in full rather than as the two borderlines alone: the two borderline project-vs-practice cases in the 31 (admitted, stated); the three "thinking of" intentions of the held-out 51 admitted at v19 and refused BY CLASS at v20; the first-person request admitted at v19–v20 and refused at v21; the two-sentence carrier with a lowercase second sentence admitted at v21 and refused at v22 by V-WHOLE-SENTENCE; draw 3 held-out 0 of 34 and draw 4 held-out 0 of 115 admitted | `test_a_declared_one_time_instruction_is_refused_not_stored_as_a_procedure`, `test_the_round_2_grammar_refusals_end_to_end`, `test_the_actor_present_grammar_refuses_reports_rejections_aspect_norms_and_requests`, `test_the_positive_form_refuses_every_unenumerated_head`, `test_the_round_3_passage_selections_are_refused_end_to_end`, `test_the_first_sentence_admits_while_the_declared_second_instruction_is_not_carried`, `test_the_bare_procedural_pool_never_passes_the_grammar` (`tests/test_0037_capture.py`); reuses 0037's invariants rather than restating them. *v6.1 asserted the route was closed with no mechanism providing it — the reviewer's return; v6.2 named a mechanism that refused the imperative and admitted the first-person request — the second return; v1 asserted "no relation denotes a practice", false on descriptions.* Mutants: store the one-time instruction as a procedure; admit "I request that you…"; admit the two-sentence carrier | CI |
...
```
corpus sha256: 56268a565e5994dda726dd60504fff6c429eebd5e73d66b021c45268b523cb18
```
...
oracle manifest sha256: 13c562360c54928b7016e1cec10af702b45cdf54858e151ff87ff4adb8874c96
