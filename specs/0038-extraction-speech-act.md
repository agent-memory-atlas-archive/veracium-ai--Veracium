...
- **0037** — procedural records and the `basis` axis. **This spec consumes 0037 directly and v2 never said so (round-1 R1-5):** §1a's cause B rested, at v1–v6.0, on **V-EXTRACTOR-BLIND** keeping every `relation_kind="procedural"` relation out of the vocabulary rendered to the prompt, which is why an instruction had nowhere legal to go; since 0037 v16 (**V-EXTRACTOR-QUOTE-GATED**) a ROUTINE the user states has one — the quote-gated procedural triple — and an instruction that is not a routine still has none, because 0037 v19's V-ACTOR-PRESENT refuses it before the carrier exists (v6.2; the reviewer's one-time instruction is the test); `record_procedure` is the governed path this spec declines to duplicate; and §2b's drop is correct ONLY while 0037's blindness holds. **0037 is ACCEPTED** (external round 5, 2026-09-07). If V-EXTRACTOR-BLIND is ever relaxed, §2b must be re-decided — V-NO-PROCEDURAL-IN-PROMPT-VOCAB fails first and says so.
...
instruction that is not a routine still has none because 0037 v19's actor-present
grammar refuses it (v6.2).* So an instruction has nowhere legal to go, and the
...
contract, the actor-present grammar, the grounding) — keyed on the relation's
...
one-time instruction, which the actor-present grammar refuses; a DECLARATIVE triple restating
...
| **V-NO-PROCEDURAL-IN-PROMPT-VOCAB** (REWRITTEN v6.2) | cause B's premise, with its MECHANISM named: the vocabulary handed to the prompt carries the registered procedural relation (0037 v16), and a triple under it reaches storage only through 0037 §4a-iii's ordered gates — in particular V-ACTOR-PRESENT, which refuses an imperative or one-time instruction ("Please review the invoices today") because it does not open with the user as the actor in the present or habitual; so an instruction that is not a routine has no storage route BECAUSE that gate refuses it, and §2b's refusal stands for declarative triples; the residual is 0037's measured one (two borderline project-vs-practice cases in 31) | `test_a_declared_one_time_instruction_is_refused_not_stored_as_a_procedure`, `test_the_actor_present_grammar_refuses_reports_rejections_aspect_norms_and_requests`, `test_the_bare_procedural_pool_never_passes_the_grammar` (`tests/test_0037_capture.py`); reuses 0037's invariants rather than restating them. *v6.1 asserted the route was closed with no mechanism providing it — the reviewer's return; v1 asserted "no relation denotes a practice", false on descriptions.* Mutant: store the one-time instruction as a procedure | CI |
...
```
corpus sha256: b930a2632c45973869740abd9f4b9897c5919180d5430722882de73cb60394f9
```
...
oracle manifest sha256: 53b9378c5f8d80b64d404a71d6df5e28af26aa9389ae585cb96330adf0bfbe36
