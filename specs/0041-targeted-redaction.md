...
| the frozen pre-restriction fixture | `specs/evidence/0041/pre_restriction.sqlite`, sha256 **`047f0d11…`**, recorded in full in `pre_restriction_manifest.json` beside the store schema version (14) and the HEAD it was frozen at (`ead0bcc9`). 🔴 **NINE shapes written through today's write paths** — including the active-episode-with-no-reason case the reviewer reproduced, and the historical source-revocation row carrying prose that round 6 required. ⚠️ **THE BYTE SIZE IS DELIBERATELY NOT PINNED HERE, and the reason is a trap worth stating: three shapes were added and the file is 188,416 bytes BEFORE AND AFTER.** SQLite allocates by page (4,096 × 46), so **a size pin does not merely go stale silently — IT CAN STAY TRUE WHILE THE ARTIFACT IT DESCRIBES IS ENTIRELY DIFFERENT.** The digest is the fingerprint; the size is an allocation detail. A test asserts the bytes against the manifest, **so a silent regeneration after the closure lands shows up as a digest change** — which is what makes the evidence rule of §4h(iii) enforceable rather than advisory |
