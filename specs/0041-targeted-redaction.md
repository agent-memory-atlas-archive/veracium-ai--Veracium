...
| 🔴 **`source_revocations.reason`** | 🔴 **a bare `str` carrying THE CALLER'S PROSE, with no vocabulary at all.** Executed: `revoke_source(..., "<the caller's sentence>", ...)` stores that sentence **verbatim** in the revocation row, while the affected edge and episode get `revoked_source` | 🔴 **v11 — TWO RULES AT TWO TIMES, because v10 collapsed them into one and thereby amended D1 without saying so (round-5 finding 2).** **(a) FUTURE WRITES: the field closes to a vocabulary, per D1**, which names it — the revocation's own reasons (`policy`, `subject_request`, `erroneous_capture`, `legal_obligation`), enforced at the `revoke_source` write path and at import, refusing loudly. **(b) PROSE ALREADY STORED: replaced at redaction like any other content carrier**, three-case as above. **Removing historical prose does not constrain what future writes may store, and having no vocabulary TODAY is the implementation work D1 records — not evidence that none is required.** ⚠️ **The vocabulary's MEMBERS are a proposal; D1 is the owner's and the second seat is putting the alternative — an explicit decision changing D1 — to him directly. This row does NOT assume D1 moves** | D1 (§9b), which names this field; §2's column table, which classed it a carrier |
...
PASSES.** `specs/evidence/0041/xfail_mutant_campaign.py` runs ten: the
reviewer's three, four more, and **three positive controls that install a
correct implementation and require green**. Ten of ten behave.

**Evidence rows this section relies on:**

| | |
|---|---|
| the §4h corollary, executable | `tests/test_0041_treatment_matrix.py::test_rows30_49_on_a_frozen_record_absence_survives_and_prose_does_not` — absent stays absent with `active` AND `assertable` unchanged; registered stays registered; only prose becomes the registry value |
| the frozen pre-restriction fixture | `specs/evidence/0041/pre_restriction.sqlite`, 188,416 bytes, sha256 in `pre_restriction_manifest.json` beside the store schema version and the HEAD it was frozen at. **Six shapes written through today's write paths**, including the active-episode-with-no-reason case. A test asserts the bytes against the manifest, **so a silent regeneration after the closure lands shows up as a digest change** — which is what makes the evidence rule of §4h(iii) enforceable rather than advisory |
