...
**T is validated and normalized by THE RESOLUTION'S OWN GATE
(`asof/resolve.py::normalise_T`) BEFORE it is compared to `now`, and the order is
load-bearing.** A naive datetime compared against an aware `now` raises
`TypeError` at runtime in Python — a failure that is neither of this spec's named
outcomes and reaches the caller as a bare interpreter error. Validating first
makes the malformed case a specified `ValueError`.

***v15 — THE MECHANISM NAMED HERE THROUGH v14 WAS FALSE OF THE SHIPPED HELPER,
and it survived seven external rounds.*** Until v15 this section attributed both
refusals to `as_utc_required` (`schema.py:41`). **That helper ADMITS both inputs
this section refuses**, and by design: it takes a **naive** value as UTC (0032's
naive-means-UTC convention for STORED values) and it **PARSES ISO-8601 TEXT**
(0030's raw-carrier normaliser, because the classifier consumes raw journal
payloads where timestamps arrive as strings). Measured against the shipped
function: a naive `datetime` returns UTC-aware, `"2026-01-01T00:00:00Z"` returns
UTC-aware, a non-datetime raises **`TypeError`** and `None` raises **`TypeError`**
— so of the four cases the old attribution named, it was wrong about all four.

***Why it survived: the false mechanism is PARTIALLY SATISFIABLE.***
`as_utc_required("not-a-date")` really does raise `ValueError` — so a test written
straight from the old row, using an obviously-bad string, **passes** — while the
input that actually matters, a **well-formed ISO string**, is admitted silently.
**A wrong mechanism that the obvious test confirms is worse than one that fails,
because the passing test is then cited as evidence for the wrong sentence.**

**The OUTCOMES in the table below were and are correct; only the mechanism was
misattributed.** The resolution's own gate enforces them: `None` → `TypeError`;
a `str` or any non-datetime → `ValueError`; a **naive** datetime → `ValueError`
**before any comparison**; an aware datetime → converted to UTC. **`as_utc_required`
is the STORE'S normaliser for values already trusted to be timestamps; T is
UNTRUSTED INPUT, and the two jobs are not the same function.** *Name the
mechanism, not just the clause: a mechanism cited by name reads as verified, and
nobody had run it.*

| input | outcome |
|---|---|
| naive `datetime` (no tzinfo) | `ValueError` from **the resolution's gate** (`asof/resolve.py::normalise_T`, v15 — NOT `as_utc_required`, which admits a naive value as UTC) — **distinct from `FutureAsOfRefused`** |
| a `str`, or any non-datetime | `ValueError` from **the resolution's gate** (`asof/resolve.py::normalise_T`, v15 — NOT `as_utc_required`, which PARSES a well-formed ISO string and raises `TypeError`, not `ValueError`, for a non-datetime) (0030 V-NORM-TOTAL; the `11c476c` str-in-datetime lesson) |
...
| `redacted` | `EXCLUDED` | *(not returnable)* |

> **`redacted` (0041 rows 30/49, implemented 2026-09-19):** a rights-driven removal on `revoked_source`'s precedent — an
> as-of read never serves a tombstone as a historical answer. The row is generated by the shipped registry; this note is the
> reason beside it.
...
| **redacted** | EXCLUDED | `NOT_RETURNABLE` | `redacted-excluded` |
...
|---|---|---|---|
| **the store cannot name a REACHABLE absorber** — a legacy row (no ledger row names the edge), **OR** every row that does names a survivor **outside the caller's view** (v15) | **YES — the only one** | `INDETERMINATE` | `absorber-unreachable-legacy` — the store CANNOT tell. **The cause string is IDENTICAL for both conditions ON PURPOSE: a scope-flavoured cause would let a principal without scope tell a HIDDEN absorber from a MISSING one, which is the existence signal V-NO-EXISTENCE-SIGNAL forbids.** The word *legacy* is therefore deliberately narrower than the condition this row covers, and **this cell says so, so that nobody later 'corrects' the name into a leak.** Implemented as: the ledger read keeps only rows whose survivor is visible in the view; none surviving → `INDETERMINATE` with this cause |
...
- **Under `as_of`, the wiki, episode and CONTESTED sections are OMITTED**
  (internal review F4; **contested added at v15, found at implementation**).
  `_recall` renders the wiki and CURRENT episodes beside the edges; a T-answer
  carrying a now-wiki and now-history would assert present truth inside a past
  answer, which is the incoherence this spec exists to avoid. Omission is chosen
  over an explicit "current, not as-of" label because a label relies on the
  reader honouring it. **The contested block is the same defect in a third
  carrier, and the list was short by one: contention is DERIVED FROM THE CURRENT
  REFUSAL SET (0003's live-refusal surface), so it is a now-fact by
  construction — and building it under `as_of` would run the recall path's
  `assertable` on the as-of branch, which is exactly what V-ONE-CLOCK's second
  mutant forbids.** The omission is therefore not a preference: **the
  alternative fails a named invariant.** Implemented as `Recall.contested == []`
  under `as_of` — **empty, not absent, so the field's type does not change.**
  ***This list is now the THREE sections `_recall` renders beside the edges. Any
  fourth renderer added later inherits the same question, and this bullet is
  where it must be answered.***
...
|---|---|---|---|
| **V-COMPAT** | `as_of=None` is byte-identical to today's recall | differential test over the existing recall suite | **CHECKED** (implemented 2026-09-08) `tests/test_0028_resolution.py::test_v_compat_as_of_none_takes_the_pre_existing_path_unchanged` |
| **V-NO-UPGRADE** | no §4b row returns an outcome stronger than 0030's verdict allows | per-row assertion against 0030's classification | **CHECKED** (implemented 2026-09-08) `tests/test_0028_resolution.py::test_v_no_upgrade_no_row_outranks_the_0030_verdict` — 🔴 **and, since 2026-09-15 (v15.1), LOAD-BEARING:** research's mutation run showed the cap fires in the suite yet the full suite stayed green with it disabled, because that test constructs only rows whose verdict is GROUNDED; `tests/test_0028_v_no_upgrade_load_bearing.py` constructs a grounded-reason row under a FENCED_AS_OF verdict for each grounded reason (asserting the precondition) and the uncapped half, and was verified RED under both the disabled and the inverted cap before it was committed (one cell per registered reason) |
| **V-NEVER-BYPASS** | for every edge with reason ∈ {corrected, disputed, revoked_source}, no T yields an assertable value — sampled inside the interval and at both boundaries | sampled-T test, mirrors 0030's V-NEVER | **CHECKED** (implemented 2026-09-08) `tests/test_0028_resolution.py::test_v_never_bypass_fenced_reasons_yield_no_assertable_value_at_any_t` (corrected, disputed, revoked_source; sampled inside and at both boundaries) |
| **V-TOTAL** | `set(RESOLUTION) == set(DISPOSITIONED_REASONS)` | registry equality test — an eighth reason fails BOTH this and the registry's own totality test, and must be dispositioned **twice** | **CHECKED** (implemented 2026-09-08) `tests/test_0028_resolution.py::test_v_total_the_resolution_table_equals_the_registry` |
| **V-MUTANT** | an edge whose `invalidation_reason` is outside the registry resolves `NOT_RETURNABLE` | **planted mutant**, asserted — a fail-closed that is narrated rather than asserted is how one silently stops holding | **CHECKED** (implemented 2026-09-08) `tests/test_0028_resolution.py::test_v_mutant_a_reason_outside_the_registry_is_not_returnable` |
| **V-NONE** | `invalidation_reason is None` on an invalidated edge resolves `NOT_RETURNABLE` | typed-None test; the table is total over the TYPE, not over today's writers | **CHECKED** (implemented 2026-09-08) `tests/test_0028_resolution.py::test_v_none_a_none_reason_on_an_invalidated_edge_is_not_returnable` |
...
| **V-SCOPE-DIFFERENTIAL** (R3-3) | `facts_valid_at` reproduces recall's principal-dependent decision | two principals, ONE store, a record visible to only one, the same `(subject, relation, T)`: the holder sees it at every T its interval covers, the other at none. **Mutant: drop `principal`/`policy` from the signature** — both principals get one answer, which is v8's declared shape | **CHECKED** (implemented 2026-09-08) `tests/test_0028_resolution.py::test_two_principals_one_store_one_record` — research's round-4 design executed as written: three assertions, the third (orthogonality: B's answer empty at EVERY T, inside, both boundaries, outside) plus the non-`Identity` refusal. *v10 cited it in the node column while it was a design, where it read as a check that runs.* |
...
**0 internal round(s) and 7 external round(s) with a returned VERDICT are recorded for `0028`; 7 package(s) were dispatched** — counted from `specs/reviews.py`, which is the source this block is generated from. A round appearing here and not there, or the reverse, is impossible by construction. **SENT rows are dispatch records, not outcomes**, and are labelled below so the two are never summed.

| round | date | findings raised (from `raised=`) | verdict (compressed) |
|---|---|---|---|
| external 1 (SENT) | 2026-08-31 | — | SENT (round-1 package 0028-round1-review-package.tar.gz sha256 481db5ab650891f681648eaecf81cf815e8e01a69f2d334b3d98da1689f5b82c — no PIN.txt in that package, the protocol predates it; the spec at v1, adopted d7bff7a, amended pre-dispatch d7bf16b) |
| external 1 (verdict) | 2026-08-31 | 7 | RETURN 0028 v1 — seven blocking findings, every one verified real against the shipped code before any fold; the owner PAUSED the arc ("hold for the bigger shape": the transaction-time carrier 0029 and the time-relative classification 0030 specced first). The verdict text was never banked verbatim; t… |
| external 2 (SENT) | 2026-09-06 | — | SENT (round 2: the v2 spec dispatched as a BARE FILE, sha256 21ffcf001d8e63717c26da7885465e3d1d87745c5f419fa835c9995d8268ba40 — never a package, so no pin, no CI run and no member count; the digest is the whole identifier) |
| external 2 (verdict) | 2026-09-06 | 5 | RETURN 0028 v2 — five blocking findings (labelled R1-* in v3–v7 and corrected to R2-* at v8: round 1 is the 2026-08-31 review). The verdict text was never banked verbatim; the findings are recorded in the version cell |
| external 3 (SENT) | 2026-09-07 | — | SENT (round-3 package c9f83b236b2f7e0b591eb18e364339f0ce6ccbe90c94b86ab259b1f525191d4f @ pin f9379abb8bdd089b92c0cc204c51fb35c16bd5a9, CI 34088697413; v8) |
| external 3 (verdict) | 2026-09-07 | 7 | RETURN for amendment 0028 v8 — four blocking (R3-1..R3-4) and three required corrections (C5–C7); "the two-protocol disclosure resolved the collected_header.json concern"; banked outbox/0028-round3-verdict-verbatim.md body sha16 49cb04b85e330e48. PLUS the round-3 SUPPLEMENTAL verdict on the two-conn… |
| external 4 (SENT) | 2026-09-07 | — | SENT (round-4 package e1b96872dc8737c3bbaa213a56c15546afa19a89f583f22dfd7cdba96dbd8735 @ pin 87280dad1012470c0f70b6e24b152c162cd1d6db, CI 34131290529; v10; a first assembly 60d8d3f8388594f9e23b37aed7776f887d164610963be63e3560ee5e2151d5da was discarded before announcement and disclosed) |
| external 4 (verdict) | 2026-09-07 | 8 | RETURN for amendment 0028 v10 — five blocking (R4-1 the typed successor result leaked existence across scope; R4-2..R4-5 old normative carriers surviving their corrections) and three minor corrections; banked outbox/0028-round4-verdict-verbatim.md body sha16 343ce4ece1910312 |
| external 5 (SENT) | 2026-09-07 | — | SENT (round-5 package a0da2aa26281b0e02a52709f1a670c3cb1fb344482219e287d7b1bf4bdc595ed @ pin b4a680c79be1de8e4b1fda8a31294c45ad12b243, CI 34141038484; v11) |
| external 5 (verdict) | 2026-09-07 | 7 | RETURN for amendment 0028 v11 — three blocking (R5-1 the accessor read the queried edge unfiltered; R5-2 the malformed-reference rule directionally inconsistent; R5-3 the checklist contradicted the cause-free contract) and four evidence/document corrections; "the archive and pinning protocol are sou… |
| external 6 (SENT) | 2026-09-07 | — | SENT (round-6 package 415d906c9102d9b1c5531b5f4da05f44c671f381e220f0c4e4987eb8e8e3cc1d @ pin eddcb2337d07b47c9cdfcb6cbb4834b5acf185f5, CI 34149633519; v12) |
| external 6 (verdict) | 2026-09-07 | 4 | RETURN for amendment 0028 v12 — two blocking (R6-1 §4b's outcome definition still required a cause for every INDETERMINATE; R6-2 §4b-o/§5/§7 described the pre-0029 bare lock form while the sealed test asserted the wrapped one) and two corrections; "the round-5 behavioral findings are correctly resol… |
| external 7 (SENT) | 2026-09-07 | — | SENT (round-7 package 846db40822ae68068e92d459e76ca40983b5920d38181ba9ca332a23752b2f1e @ pin 6680150e510f9c04f9439793671565e9e2a0e516, CI 34154534452; v13; a first assembly ce1bc53954e1c6a15c4e8b735e4ec2b24dbbdeefb268fce77620b0feba5fe283 was refused by the seal check's first real stale sweep, discar… |
| external 7 (verdict) | 2026-09-07 | 0 | ACCEPT — "Round 7 resolves both round-6 blockers … The two editorial corrections are also complete." The frozen invariant surface is every §6 row (nineteen names, none omitted). "Items marked OWED remain implementation obligations; acceptance does not assert that the unwritten facts_valid_at, read_w… |

**Per-finding closure ledger — PROCESS §4a.** **38 finding(s) for `0028`** — every number here is DERIVED from the rows below (external round 7, R7-1: the manifest claimed 26 while the ledgers held 31, and 0023 said 9/9 above a 10-row table); the total across the tracked specs is derived once, in `specs/STATUS.md`. Generated from `specs/closure_findings.py` and validated against `specs/reviews.py` on `(spec, kind, round, id)` EXACTLY — extras, duplicates, wrong rounds and empty evidence all fail the build.

| finding | round | what it was | closed in | evidence (runnable) |
|---|---|---|---|---|
| **0028-R1-1** | external 1 | The registry holds SEVEN reasons (revoked_source, 0022's seat); the spec's "closed six" failed verification against DISPOSITIONED_REASONS | v2–v7 (landed 26453b6): the §4b table is GENERATED from AS_OF_DISPOSITION in registry order with totality asserted at generation; V-CROSS compares it to the enum as data | `$PY -m pytest tests/test_0028_reason_resolution_table.py::test_check_passes_against_the_spec_table_as_data tests/test_0028_reason_resolution_table.py::test_mutant_eighth_reason_fails_totality -q -p no:randomly` |
| **0028-R1-2** | external 1 | known_as_of is unimplementable from the stated fields: invalidated_at is a valid-time endpoint, there is no transaction-time carrier (the backdated-correction counterexample) | the arc PAUSED on the owner's ruling; 0029 (accepted) supplies the transaction-time carrier; v2 (landed as v7, 26453b6) is valid-time only and says so in its title | `git show 26453b6 -- specs/0028-as-of-query.md` |
| **0028-R1-3** | external 1 | STRUCTURAL: Edge.assertable requires active, so no historical edge can ground under the unchanged classifier | 0030 (accepted) supplies the time-relative classifier classify_as_of; v2 (26453b6) reads through it and never through Edge.valid_now/assertable — the absence proof | `$PY -m pytest tests/test_0028_asof_absence.py::test_the_behavioural_form_runs_the_shipped_asof_tests_under_the_clock tests/test_0028_asof_absence.py::test_the_static_form_reaches_the_roots_and_finds_no_predicate_access -q -p no:randomly` |
| **0028-R1-4** | external 1 | Chain/arity mechanics undefined: backward supersedes, note-carried absorption links, the absorbed-to row unreachable through §4a | v2–v7 (26453b6): §4b-iii reachability stated per row; later the successor contract §5.1 (v3→v13) with the direct-successor rule | `git show 26453b6 -- specs/0028-as-of-query.md` |
| **0028-R1-5** | external 1 | Correction chains can cross scope (source identity, not subject; 0020 owns read visibility) | v2–v7 (26453b6): scope before the historical filter; later the NON-REVEALING successor design and V-SCOPE-DIFFERENTIAL (v11–v13) | `$PY -m pytest tests/test_0028_successor_lookup.py::test_the_model_holds_the_observation_table_and_both_invariants -q -p no:randomly` |
| **0028-R1-6** | external 1 | The recall baseline is misstated (active_only=False history already flows) and V-DET overclaims for the composed path | v2–v7 (26453b6): the baseline restated; determinism stated over the composed path with the injected clock (V-ONE-CLOCK) | `git show 26453b6 -- specs/0028-as-of-query.md` |
| **0028-R1-7** | external 1 | The result/gap carriers and the acceptance corpus are not finite | v2–v7 (26453b6): §6a made FINITE — every invariant CHECKED by a named node or OWED at implementation, no count in prose | `git show 26453b6 -- specs/0028-as-of-query.md` |
| **0028-R2-1** | external 2 | §5.1 was cited four times and did not exist — the successor accessor had no contract | v3 (landed 26453b6): §5.1 written — direct successors only, scope before the historical filter; redesigned at the view v11–v13 and modelled executably | `$PY -m pytest tests/test_0028_successor_lookup.py::test_the_model_holds_the_observation_table_and_both_invariants -q -p no:randomly` |
| **0028-R2-2** | external 2 | The verdict column used GROUNDED, a name shipping in neither vocabulary | v3 (26453b6): the table generated from AS_OF_DISPOSITION; V-CROSS compares it to the enum as data | `$PY -m pytest tests/test_0028_reason_resolution_table.py::test_check_passes_against_the_spec_table_as_data tests/test_0028_reason_resolution_table.py::test_mutant_shuffled_rows_diverge_from_the_spec_table -q -p no:randomly` |
| **0028-R2-3** | external 2 | T > now was declared clamp OR refuse, both conforming — 0011 R1-4's unconformable shape | v4 (26453b6): closed on the owner's ruling, verbatim "refuse" — the typed FutureAsOfRefused carrying T and now; clamp WITHDRAWN | `git show 26453b6 -- specs/0028-as-of-query.md` |
| **0028-R2-4** | external 2 | Three of §4b-iii's four absorbed-duplicate rows were unreachable (canonical absorption is empty-interval by construction) | v3 (26453b6): the rows named as the class no shipped writer produces; the model's forward_missing states carry the same class | `git show 26453b6 -- specs/0028-as-of-query.md` |
| **0028-R2-5** | external 2 | The inventory said ten checks where §6 has thirteen | v3 (26453b6): no count in prose; the CHECKED/OWED markers are the one accounting rule (re-caught at round 5) | `git show 26453b6 -- specs/0028-as-of-query.md` |
| **0028-R3-1** | external 3 | The old two-clock algorithm remained in §4a, the normative procedure, after two folds removed it elsewhere | v9 (0410229): §4a re-derived — one injected clock captured at step 0, no step reads a clock; the absence proof over classify_as_of (static tree + recording clock) | `$PY -m pytest tests/test_0028_asof_absence.py::test_the_behavioural_form_runs_the_shipped_asof_tests_under_the_clock tests/test_0028_asof_absence.py::test_no_shipped_path_reaches_the_classifier_so_the_derived_set_is_the_reachable_set -q -p no:randomly` |
| **0028-R3-2** | external 3 | edges_superseding cannot return its promised omission information | v9: a typed result — which R4-1 then found to be an existence signal; v11–v13: the cause-free three-value SuccessorLookup, modelled executably | `$PY -m pytest tests/test_0028_successor_lookup.py::test_v9_typed_result_reintroduces_the_existence_signal -q -p no:randomly` |
| **0028-R3-3** | external 3 | facts_valid_at promises scoped behaviour without carrying scope | v9 (0410229): principal and policy on the read API (keyword-only), scope through the shared ScopeView | `git show 0410229 -- specs/0028-as-of-query.md` |
| **0028-R3-4** | external 3 | MCP exposure remained an unresolved product-interface decision | v9 (0410229): Q5 closed — the as-of read is NOT exposed on the MCP surface in this version; the sentence stated once (R4-4 caught a surviving contrary carrier, fixed v11) | `git show 0410229 -- specs/0028-as-of-query.md` |
| **0028-R3-C5** | external 3 | The standalone generator command did not run from the extracted tree without PYTHONPATH (a fixed development-machine path) | v9: the generator derives the repository root; the round-4 verdict confirmed it runs without PYTHONPATH | `$PY -m pytest tests/test_0028_reason_resolution_table.py::test_check_passes_against_the_spec_table_as_data -q -p no:randomly` |
| **0028-R3-C6** | external 3 | Remove the disclosed stale numeral in the v8 history rather than changing it from six to eight | v9 (0410229): removed as the package recommended | `git show 0410229 -- specs/0028-as-of-query.md` |
| **0028-R3-C7** | external 3 | Clarify the rollback-journal concurrency wording: a writer may wait and commit after the window, or receive a busy error — "refused for the whole resolution" names an outcome that does not occur | v10 (87280da) from the two-connection transcript the reviewer reproduced (the supplemental verdict); the measured rows in §5 | `$PY -m pytest tests/test_0028_window_transcript.py::test_default_mode_writer_waits_and_commits_after_a_window_shorter_than_its_timeout tests/test_0028_window_transcript.py::test_default_mode_writer_fails_and_loses_the_write_when_the_window_outlasts_its_timeout -q -p no:randomly` |
| **0028-R4-1** | external 4 | SuccessorLookup contradicted scope indistinguishability — the typed result added to fix R3-2 was itself an existence signal across the scope boundary | v11 (b4a680c): the NON-REVEALING design — a successor outside the view is not an omission; V-NO-EXISTENCE-SIGNAL with whole-object equality and its positive control; the registry NAMES_A_SUCCESSOR | `$PY -m pytest tests/test_0028_successor_lookup.py::test_v9_typed_result_reintroduces_the_existence_signal tests/test_0028_successor_lookup.py::test_collapsing_the_hidden_case_into_head_trades_a_leak_for_a_lie tests/test_0028_successor_lookup.py::test_the_hand_set_corrected_alone_lies_to_the_principal_on_two_reachable_states -q -p no:randomly` |
| **0028-R4-2** | external 4 | A second wall-clock exception remained ("except the current-truth pointer") | v11 (b4a680c): no step reads a clock; the pointer is a current-state fact evaluated at the same captured now | `git show b4a680c -- specs/0028-as-of-query.md` |
| **0028-R4-3** | external 4 | The old rollback-journal rule remained in V-ONE-SNAPSHOT (a third wording of the disproved claim) | v11 (b4a680c): V-ONE-SNAPSHOT restated from the measured rows; the wording registered as withdrawn (v13) | `$PY -m pytest tests/test_0028_window_transcript.py::test_wal_writer_commits_during_the_window_and_the_reader_keeps_its_snapshot -q -p no:randomly` |
| **0028-R4-4** | external 4 | MCP remained simultaneously closed and open | v11 (b4a680c): one sentence, Q5 closed, every contrary carrier removed | `git show b4a680c -- specs/0028-as-of-query.md` |
| **0028-R4-5** | external 4 | Pointer-walk text still consumed the old list return | v11 (b4a680c): the walk reads the SuccessorLookup result; resolved together with R4-1 | `git show b4a680c -- specs/0028-as-of-query.md` |
| **0028-R4-M1** | external 4 | The orphaned "moot." after the corrected lane statement | v11 (b4a680c) | `git show b4a680c -- specs/0028-as-of-query.md` |
| **0028-R4-M2** | external 4 | Several §6 rows described future checks while §6a said every invariant already carried a named check | v11 (b4a680c): §6a stated by class — CHECKED today vs OWED at implementation | `git show b4a680c -- specs/0028-as-of-query.md` |
| **0028-R4-M3** | external 4 | The standalone table generator correction is successful (confirmation) | v9/v11: runs without PYTHONPATH | `$PY -m pytest tests/test_0028_reason_resolution_table.py::test_check_passes_against_the_spec_table_as_data -q -p no:randomly` |
| **0028-R5-1** | external 5 | The public successor accessor could disclose a hidden queried edge — the queried edge was read from the unfiltered collection | v12 (eddcb23): the queried edge's assertion is readable only when visible in the caller's view; edges_superseding is not an existence oracle; the differential on both arms | `$PY -m pytest tests/test_0028_successor_lookup.py::test_the_v11_lookup_that_read_the_queried_edge_unfiltered_discloses_it tests/test_0028_successor_lookup.py::test_collapse_everything_passes_the_equality_half_and_fails_the_control -q -p no:randomly` |
| **0028-R5-2** | external 5 | The malformed-reference rule was directionally inconsistent with an accessor searching supersedes == edge_id | v12 (eddcb23): forward_missing and dangling_backward named apart; both in the model's table | `$PY -m pytest tests/test_0028_successor_lookup.py::test_the_model_holds_the_observation_table_and_both_invariants -q -p no:randomly` |
| **0028-R5-3** | external 5 | The reviewer checklist contradicted the cause-free result contract | v12 (eddcb23): the checklist carries two classes; R6-1 then found the definition it summarises unchanged (closed v13) | `git show eddcb23 -- specs/0028-as-of-query.md` |
| **0028-R5-C1** | external 5 | README and §9 claimed nine states while EXPECTED executed eight | v12 (eddcb23): no count in any carrier; the program prints it | `git show eddcb23 -- specs/0028-as-of-query.md` |
| **0028-R5-C2** | external 5 | The model header said supersession is asserted when the reason equals only corrected, contradicting the three-reason registry | 808d5e4: the header corrected to the registry | `git show 808d5e4 -- specs/evidence/0028/check_successor_lookup.py` |
| **0028-R5-C3** | external 5 | The test and model headers still described six states | 808d5e4 + eddcb23 (build()'s docstring): no count in either header | `git show 808d5e4 -- specs/evidence/0028/check_successor_lookup.py` |
| **0028-R5-C4** | external 5 | §6a said nineteen invariants and then "no count is stated here, deliberately" | v12 (eddcb23): one accounting rule — the CHECKED/OWED markers | `git show eddcb23 -- specs/0028-as-of-query.md` |
| **0028-R6-1** | external 6 | §4b's outcome definition still required a cause for every INDETERMINATE, contradicting cause-free SUCCESSOR_UNAVAILABLE (the checklist was fixed at v12, the definition it summarises was not) | v13 (6680150): §4b carries the two classes — causes for conditions observable within the view; none for SUCCESSOR_UNAVAILABLE, by design | `git show 6680150 -- specs/0028-as-of-query.md` |
| **0028-R6-2** | external 6 | §4b-o, §5 and §7 described the commit-time lock as SQLite's bare message "surfacing unwrapped" while the sealed tree (0029 v11) and the window test carried the wrapped form | v13 (6680150): all three carriers state the wrapped refusal naming the COMMIT site, SQLite's message as the cause; history marked; the wording registered as withdrawn | `$PY -m pytest tests/test_0028_window_transcript.py::test_default_mode_writer_fails_and_loses_the_write_when_the_window_outlasts_its_timeout -q -p no:randomly` |
| **0028-R6-C1** | external 6 | §9 carried a malformed duplicated phrase (the round-5 "no count" replacement fused with its neighbour) | v13 (6680150) | `git show 6680150 -- specs/0028-as-of-query.md` |
| **0028-R6-C2** | external 6 | The model's introduction called itself "the design v11 specifies" while its queried-edge behaviour is the v12 amendment | 6680150: "the v11 design as amended by v12" at both docstring sites; the phrase at zero over the file | `git show 6680150 -- specs/evidence/0028/check_successor_lookup.py` |
